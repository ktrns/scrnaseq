...
parser = ArgumentParser(
  ...
  epilog="Demultiplexing by sample has to be run first and the results must be in a subdirectory 'demultiplexed'. The script will then read the metadata.tsv.gz files per sample and adjust the cell 
barcodes suffixes (e.g. '-1' will become '-2' for the second sample) in the cellranger aggregate results accordingly. It will also update the CSV file with the cell identities for import into the 
Loupe Cell Browser. Important: The script needs to be run in the same directory as cellranger aggregate!")

parser$add_argument(c("--aggregate-results"), action="store", help="Path to an outs directory generated by cellranger aggregate", dest="aggregate_results")
