...
sc = ReadSparseMatrix(params$path_data, project=params$project_id, row_name_column=1, hto_names=params$hto_names)
...
p = VlnPlot(sc, features=nfeature_metrics, idents=levels(Seurat::Idents(sc)), pt.size=0) + 
  ...
  AddStyle(title="Number of features for HTO-classified cells", 
           fill=hto_colours$col_hto_collapsed, 
...
sc.all = prelim.analysis(sc=sc.all, mt=params$mt_names, pc_n=10)
sc = prelim.analysis(sc=sc, mt=params$mt_names, pc_n=10)
```
...
loupe_meta = loupe_meta[,c("HTO_classification", "HTO_classification.global")]
