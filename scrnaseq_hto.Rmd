...
author:
  ...
  - "This report is based on and inspired by: https://satijalab.org/seurat/v3.1/hashing_vignette.html"
date: "June 2020"
geometry: margin=2cm
output:
   html_document:
...
`r if(!is.null(params$sample_cells)) print("**Sample data to at most n cells (mainly for tests); set to NULL to deactivate**  ")`
`r if(!is.null(params$sample_cells)) params$sample_cells`

```{r hto_colours}
params$col_hto_global = ggsci::pal_npg()(3)
params$col_hto_all = ggsci::pal_npg()(factorial(length(params$hto_names)) + 1)
params$col_hto_collapsed = ggsci::pal_npg()(length(params$hto_names) + 2)
hto_levels = c("Negative", "Doublet", params$hto_names[length(params$hto_names):1])
names(params$col_hto_collapsed) = hto_levels
```
...
sc = Read10xDataset(params$path_data, project=params$project, row_name_column=1, hto_names=params$hto_names)

# If requested: sample at most n cells
if (!is.null(params$sample_cells)) {
  sampled_barcodes = sample(Seurat::Cells(sc), min(params$sample_cells, length(Seurat::Cells(sc))))
...
p1 = ggplot(sc@meta.data %>% dplyr::count(HTO_classification.global), 
  ...
  xlab("") + ggplot2::ylab("")
p1 = PlotMystyle(p=p1, title="HTO global classification results", col=params$col_hto_global)
...
p2 = ggplot(sc@meta.data %>% dplyr::count(hash.ID), 
  ...
  coord_polar("y", start=0) + 
  xlab("") + ylab("")
p2 = PlotMystyle(p=p2, title="HTO classification results", col=params$col_hto_collapsed)
...
p1 = ggplot(hto_t_raw_pseudo %>% tidyr::pivot_longer(tidyr::everything()), aes(x=name, y=value, fill=name)) + 
  ...
  xlab("") + ylab("") 
p1 = PlotMystyle(p1, title="HTO raw counts", col=params$col_hto_collapsed, legend_title="HTO")
  
p2 = ggplot(hto_t_norm %>% tidyr::pivot_longer(tidyr::everything()), 
            aes(x=name, y=value, fill=name)) + 
  geom_violin() + 
  xlab("") + ylab("")
p2 = PlotMystyle(p2, title="HTO normalised counts", col=params$col_hto_collapsed, legend_title="HTO")
...
p = ggplot(n, aes(x=value1, y=value2, color=cell_classification)) + 
  ...
  scale_color_manual(values=params$col_hto_collapsed)
p = PlotMystyle(p)
...
p = RidgePlot(sc, assay="HTO", features=rownames(Seurat::GetAssay(sc, assay="HTO")), 
              same.y.lims=TRUE, cols=params$col_hto_collapsed, combine=FALSE)
...
sc.all = prelim.analysis(sc=sc.all, mt=params$mt, pc_n=10)
sc = prelim.analysis(sc=sc, mt=params$mt, pc_n=10)
```
...
### Before filtering
```{r umap_before}
p = Seurat::DimPlot(sc.all, reduction="umap", group.by="hash.ID", cols=params$col_hto_collapsed) 
p = PlotMystyle(p, 
...
p = Seurat::DimPlot(sc, reduction="umap", group.by="hash.ID", cols=params$col_hto_collapsed) 
p = PlotMystyle(p, 
