...
params:
   ...
   sample_cells: NULL
title: "Single-cell RNA-seq data analysis of project `r params$project`"
subtitle: "HTO demultiplexing"
---
...
p1 = ggplot(sc@meta.data %>% dplyr::count(HTO_classification.global), 
  ...
  xlab("") + ggplot2::ylab("")
p1 = PlotMystyle(p=p1, title="HTO global classification results", col=hto_colours$col_hto_global)
...
p2 = ggplot(sc@meta.data %>% dplyr::count(hash.ID), 
  ...
  coord_polar("y", start=0) + 
  xlab("") + ylab("")
p2 = PlotMystyle(p=p2, title="HTO classification results", col=hto_colours$col_hto_collapsed)
...
p1 = ggplot(hto_t_raw_pseudo %>% tidyr::pivot_longer(tidyr::everything()), aes(x=name, y=value, fill=name)) + 
  ...
  xlab("") + ylab("") 
p1 = PlotMystyle(p1, title="HTO raw counts", col=hto_colours$col_hto_collapsed, legend_title="HTO")
  
p2 = ggplot(hto_t_norm %>% tidyr::pivot_longer(tidyr::everything()), 
            aes(x=name, y=value, fill=name)) + 
  geom_violin() + 
  xlab("") + ylab("")
p2 = PlotMystyle(p2, title="HTO normalised counts", col=hto_colours$col_hto_collapsed, legend_title="HTO")
...
p = ggplot(n, aes(x=value1, y=value2, color=cell_classification)) + 
  ...
  scale_color_manual(values=hto_colours$col_hto_collapsed)
p = PlotMystyle(p)
...
p = PlotMystyle(p, 
                ...
                col=hto_colours$col_hto_collapsed, 
...
# Parameter table
```{r parameters_table}
out = scrnaseq_hto_parameter_info(params$project, params$path_data, params$path_out, params$hto_names, params$mt, params$color, params$sample_cells)
