...
params:
  ...
  # Whether or not to remove cell cycle effects (true, false)
  cc_remove: !r FALSE

  # Should all cell cycle effects be removed, or only the difference between profilerating cells (G2M and S phase)? (true, false)
  # Read https://satijalab.org/seurat/v3.1/cell_cycle_vignette.html, for an explanation
  cc_remove_all: !r FALSE
   
  # Whether or not to re-score cell cycle effects after data (true, false)
  ...
  # Enrichr databases of interest (see Enrichr site for a list of possible databases)
  ...
  col_palette_samples: "ggsci::pal_npg"
   
  # Colour palette used for cluster
  col_palette_clusters: "ggsci::pal_jama"
  ...
  debugging_mode: "print_traceback"
...
library(reticulate) # required for python (e.g. "leiden" clustering)
...
# Annotation: biomaRt
# DEG: mast, limma (optional; for a more efficient implementation of the Wilcoxon Rank Sum Test according to Seurat)
...
knitr::opts_chunk$set(echo=TRUE,                     # output code
                      ...
                      dev=c(ifelse(require("ragg", quietly=TRUE), "ragg_png", "png"), "pdf"),           # create figures in png and pdf; the first device (png) will be used for HTML output
...
# Source additional R functions from the git directory
...
# Create output directories and remove content if overwrite is TRUE (except for annotation where we want to keep/re-use the files)
param$path_out = gsub("/$", "", param$path_out)
dir.create(param$path_out, recursive=TRUE, showWarnings=FALSE)
for(d in c("figures", "marker_degs", "annotation", "data", "export")) {
  d = file.path(param$path_out, d)
  dir.create(d, recursive=TRUE, showWarnings=FALSE)
  
  if (length(list.files(d)) > 0 & d != "annotation") {
    if (param$overwrite) {
      unlink(d, recursive=TRUE)
      dir.create(d, recursive=TRUE, showWarnings=FALSE)
    } else {
      stop(paste("Output directory", d, "already exists but is not empty and parameter 'overwrite' is set to false. Please either delete all files in this directory or set parameter 'overwrite' to true so that the script will delete the files."))
    }
  }
}
...
param = check_parameters_scrnaseq(parameter=param)
...
if (is.null(param$cluster_resolution_test)) param$cluster_resolution_test = c()
...
# Check enrichR
error_messages = c(error_messages, check_enrichr(databases=param$enrichr_dbs, site=param$enrichr_site))
# Check ensembl
...
for (i in seq(sc)) {
  if (!is.null(param$doublets)) {                         
    sc[[i]]@meta.data = dplyr::rename(sc[[i]][[]], 
                                      DoubletScore=param$doublets, 
                                      IsDoublet=param$doublets_prediction)
  } 
}
```
...
# Print gene metadata
knitr::kable(head(sc[[1]][[param$assay_raw]]@meta.features, 5), align="l", caption="Feature metadata, top 5 rows (only first dataset shown)") %>% 
  kableExtra::kable_styling(bootstrap_options=c("striped", "hover"))  %>% 
  kableExtra::scroll_box(width="100%")
```
...
sc = purrr::map(list_names(sc), function(n) {
  ...
  df = data.frame(num_cells_expr, num_cells_expr_threshold, counts_median)
  
  sc[[n]][[param$assay_raw]]@meta.features = cbind(sc[[n]][[param$assay_raw]]@meta.features, df[rownames(sc[[n]][[param$assay_raw]]@meta.features), ])
  return(sc[[n]])
})
```
...
genes_highestExpr = lapply(sc_names, function(i) {
  top_ten_exp = sc[[i]][[param$assay_raw]]@meta.features["counts_median"] %>% dplyr::arrange(dplyr::desc(counts_median)) %>% head(n=10)
  return(rownames(top_ten_exp))
  }) %>%
  unlist() %>%
  unique()

genes_highestExpr_counts = purrr::map_dfc(sc[sc_names], .f=function(s) s[[param$assay_raw]]@meta.features["counts_median"][genes_highestExpr, ]) 
genes_highestExpr_counts$gene = genes_highestExpr
genes_highestExpr_counts$mean = genes_highestExpr_counts %>% dplyr::select(-gene) %>% rowMeans()
genes_highestExpr_counts = genes_highestExpr_counts %>% dplyr::slice_max(mean, n=10) %>% tidyr::pivot_longer(cols=all_of(sc_names))
genes_highestExpr = unique(genes_highestExpr_counts$gene)

genes_highestExpr_counts$gene = factor(genes_highestExpr_counts$gene, levels=genes_highestExpr)
...
sc_features_to_exclude = purrr::map(list_names(sc), function(n) {
  ...
  else return(names(which(sc[[n]][[param$assay_raw]]@meta.features["num_cells_expr_threshold"] < param$feature_filter[[n]][["min_cells"]])))
})
...
fig_height_vf = 5 * ceiling(length(names(sc))/2) %>% min(100)
```
...
if (param$integrate_samples[["method"]] != "single") {
  ...
  sc_feature_metadata = purrr::map(values_to_names(assay_names), function(a) {
    ...
    contains_neccessary_columns = purrr::map_lgl(list_names(sc), function(n) { 
      all(c("feature_id", "feature_name", "feature_type") %in% colnames(sc[[n]][[a]]@meta.features)) 
      })

    if (all(contains_neccessary_columns)) {
      feature_id_name_type = purrr::map(sc, function(s) return(s[[a]]@meta.features[c("feature_id", "feature_name", "feature_type")]) )
...
if (param$integrate_samples[["method"]] == "integrate") {
  ...
  for (a in Seurat::Assays(sc)) {
    if (a %in% names(sc_feature_metadata)) {
      sc[[a]]@meta.features = sc_feature_metadata[[a]][rownames(sc[[a]]),, drop=FALSE]
    }
  }
...
sc = suppressWarnings(Seurat::FindClusters(sc, resolution=cluster_resolutions, algorithm=4, verbose=FALSE))

# Construct a phylogenetic tree relating the "average" cell from each cluster for each clustering
...
fig_height_test_clusters = fig_height_test_clusters %>% min(100)
```

```{r run_umap_test, fig.height=fig_height_test_clusters, message=FALSE}
# Default UMAP
...
```{r subclustering_9, eval=FALSE}
# Run subclustering
sc = suppressWarnings(Seurat::FindSubCluster(object=sc,
                            cluster="9",
...
p = VlnPlot(sc, features=c("Acan"), group.by="subcluster", pt.size=0) 
p

p = FeaturePlot(sc, features=c("Acan"), cols=c("lightgrey", param$col), label=TRUE)
p
```

```{r subclustering_9_save, eval=FALSE}
# If subclustering is okay confirm
sc$seurat_clusters = sc$subcluster %>% as.factor()
sc$subcluster = NULL
Idents(sc) = "seurat_clusters"
```

```{r update_clusters_meta, eval=FALSE}
# Rename clusters and update active identity
#tmp = sc$seurat_clusters %>% table() %>% sort(decreasing=TRUE) %>% as.list()
#clusters_old_to_new = as.character(1:length(tmp))
#names(clusters_old_to_new) = names(tmp)
#sc$seurat_clusters = factor(clusters_old_to_new[as.character(sc$seurat_clusters)], levels=unname(clusters_old_to_new))
#Idents(sc) = "seurat_clusters"
#rm(tmp)

# Save original cluster colours in Seurat object and reset colours for clusters
sc = ScAddLists(sc, lists=list(seurat_clusters_original=Seurat::Misc(sc, "colour_lists")$seurat_clusters), lists_slot="colour_lists")
...
```{r clusterQC_doubletScore_featurePlot}
...
```{r clusterQC_doubletYesNo_featurePlot}
if ("DoubletScore" %in% colnames(sc[[]])) {
...
fig_height_knownMarkers_dotplot = min(height_per_row_variable, 7) %>% min(100)
fig_height_knownMarkers_avgvioplot = max(height_per_row_variable, height_per_row_variable * nr_avgviolin) %>% min(100)
...
fig_height_knownMarkers_vect = max(height_per_row, height_per_row * nr_rows) %>% min(100)
```
...
```{r knownMarkers_violin, fig.height=fig_height_knownMarkers_avgvioplot}
if ((known_markers_n > 0) & any(idx_avgplot)) {
  p_list = list()
...
if (markers_found) {
  ...
  fig_height_dp_2cols = height_per_row_variable * nr_rows_dp_2cols
  
} else {
  fig_height_3cols = fig_height_dp_2cols = 7 
}
fig_height_3cols = fig_height_3cols %>% min(100)
fig_height_dp_2cols = fig_height_dp_2cols %>% min(100)
```
...
fig_height_degs_per_cl = max(5, 
                             max(2, 0.3 * (sc$orig.ident %>% unique() %>% length())) * length(levels(sc$seurat_clusters)) * 2) %>% min(100) # We multiply by 2, as the legend requires quite some space
```
...
if (markers_found) {
  ...
  p = Seurat::DoHeatmap(sc, group.by="seurat_clusters", features=markers_filt$all$gene, group.colors=param$col_clusters, label=FALSE, cells=cells_subset) +
    theme(axis.text.y=element_blank()) +
    ggtitle("Heatmap of scaled gene expression data, all genes differentially expressed between a cluster and the rest")
  suppressMessages({p = p + scale_fill_viridis_c("Expression") + scale_color_manual("Cluster", values=param$col_clusters)})
...
if (markers_found) {
  ...
  p = Seurat::DoHeatmap(sc, group.by="seurat_clusters", features=features_subset, group.colors=param$col_clusters, label=FALSE, cells=cells_subset) + 
    theme(axis.text.y=element_text(size=8)) +
    ggtitle("Heatmap of scaled gene expression data, top genes up-regulated in a cluster compared to the rest")
  suppressMessages({p = p + scale_fill_viridis_c("Expression") + scale_color_manual(values=param$col_clusters)})
  p
}
```
...
if (markers_found) {
  ...
  invisible(purrr::map(names(marker_genesets_up_enriched), function(n) {
    EnrichrWriteResults(enrichr_results=marker_genesets_up_enriched[[n]],
                        file=file.path(param$path_out, "marker_degs", paste0("functions_marker_up_cluster_", make.names(n), "_vs_rest.xlsx")))
  }))
  ...
  invisible(purrr::map(names(marker_genesets_down_enriched), function(n) {
    EnrichrWriteResults(enrichr_results=marker_genesets_down_enriched[[n]],
                        file=file.path(param$path_out, "marker_degs", paste0("functions_marker_down_cluster_", make.names(n), "_vs_rest.xlsx")))
  }))
...
openxlsx::write.xlsx(x=data.frame(seurat_id=rownames(sc)) %>%
                       dplyr::filter(seurat_id %in% names(seurat_rowname_to_ensembl)) %>%                       dplyr::mutate(ensembl_gene_id=seurat_rowname_to_ensembl[seurat_id]) %>%
