...
knitr::opts_chunk$set(fig.path=paste0(param$path_out, "/figures/"))
...
# Create output directories <==== How about we organise a bit more :-)
...
# Set up colors for samples and add to the sc objects
...
sc = purrr::map(list_names(sc), function(n) {
  sc[[n]] = ScAddLists(sc[[n]], lists=list(samples=param$col_samples[n]), lists_slot="colour_lists")
  return(sc[[n]])
})
...
sc = purrr::map(list_names(sc), function(n) {
  ...
  if (any(is.na(sc[[n]][["S.Score"]])) | any(is.na(sc[[n]][["G2M.Score"]]))) param$cc_remove=FALSE
  return(sc[[n]])
})
...
if (param$integrate_samples[["method"]]=="integrate") {
  ...
  # Run integration
  sc = RunIntegration(sc, ndims=param$integrate_samples[["dimensions"]], 
                      verbose=FALSE, 
...
```{r plot_RLE_raw}
# Plot raw data
p = PlotRLE(as.matrix(log2(GetAssayData(subset(sc, cells=cells_RLE_subset), assay="RNA", slot="counts") + 1)), 
            id=cells_RLE_sample, 
...
p = PlotRLE(as.matrix(GetAssayData(subset(sc, cells=cells_RLE_subset), assay=param$norm, slot="data")), 
            id=cells_RLE_sample, 
            col=param$col_samples) + 
...
if (! param$integrate_samples[["method"]] %in% c("single", "merge")) {
  ...
  p = PlotRLE(as.matrix(GetAssayData(subset(sc, cells=cells_RLE_subset), assay=paste0(param$norm, "integrated"), slot="data")), 
              id=cells_RLE_sample, 
              col=param$col_samples) + 
...
p = Seurat::DimHeatmap(sc, dims=1:min(20, ncol(sc)), cells=min(500, ncol(sc)), balanced=TRUE, fast=FALSE)
p
```
...
# Add enrichment
if (length(cell_samples) > 1) tbl = rbind(tbl, CellsFisher(sc))
...
col =  GenerateColours(num_colours=length(levels(sc$Phase)), palette="ggsci::pal_npg")
names(col) = levels(sc$Phase)
...
p3 = ggplot(sc@meta.data %>% 
              dplyr::group_by(orig.ident, Phase) %>% 
              dplyr::summarise(num_reads=length(Phase)), 
            aes(x=orig.ident, y=num_reads, fill=Phase)) + 
  geom_bar(stat="identity", position="fill") + 
  scale_y_continuous("Fraction of cells") +
  AddStyle(fill=Misc(sc, "colour_lists")[["Phase"]]) + 
...
output_files[["markers"]] = DegsWriteToFile(split(markers_out, markers_out$cluster),
...
# Add markers to the seurat object
...
if (markers_found) {
  ...
  fig_height_dp_2cols = nr_rows_dp_2cols * max(4, 0.5 * length(unique(Idents(sc))))
...
if (markers_found) {
  # First plot per cluster to adjust the colours accordingly, and combine it all at the end
  p_list=list()
  for(cl in levels(sc$seurat_clusters)) {
    p_list[[cl]] = Seurat::FeaturePlot(sc, features=markers_top %>% dplyr::filter(cluster==cl) %>% dplyr::pull(gene), 
                                       cols=c("lightgrey", param$col_clusters[cl]), combine=FALSE, label=TRUE)
    for (i in seq(p_list[[cl]])) p_list[[cl]][[i]] = p_list[[cl]][[i]] + 
        AddStyle(title=(markers_top %>% dplyr::filter(cluster==cl) %>% dplyr::pull(labels))[i], 
                 xlab="", ylab="", 
                 legend_position="bottom")
  }
  
  # Flatten the plot structure
...
if (markers_found) {
  ...
  # Write to files
  output_files[["markers_enrichr_up"]] = purrr::map(names(marker_genesets_up_enriched), function(n) {
  ...
  # Write to files
  output_files[["markers_enrichr_down"]] = purrr::map(names(marker_genesets_down_enriched), function(n) {
...
if (markers_found) {
  ...
  for(n in levels(marker_genesets_top_enriched$Cluster)){
    ...
    cat(knitr::kable(marker_genesets_top_enriched %>% dplyr::ungroup() %>% dplyr::filter(Cluster==n) %>% dplyr::select(Database, Term, Direction, Adjusted.P.value, Odds.Ratio, Combined.Score), 
...
# Write degs to files
output_files[["degs"]] = purrr::map_chr(degs, function(degs_subsets) {
  # The output file
...
output_files[["degs_enrichr"]] = purrr::map_chr(degs, function(degs_subsets) {
  # The output file
...
for (i in seq(degs)) { 
  for (j in seq(degs[[i]])) {
    ...
    tmp = cbind(x=x, y=y) %>% as.data.frame()
    tmp = cbind(tmp, col=rep("none", nrow(tmp)))
    tmp = cbind(tmp, gene=rownames(sc))
    lims = c(min(c(x, y)), max(x, y))
    
    ## Color DEGs
    up = degs[[i]][[j]]$results %>% dplyr::filter(avg_log2FC > 0) %>% dplyr::pull(gene)
    if (length(up) > 0) tmp[up, "col"] = "up"
    down = degs[[i]][[j]]$results %>% dplyr::filter(avg_log2FC < 0) %>% dplyr::pull(gene)
    if (length(down) > 0) tmp[down, "col"] = "down"
    cols = c(none="grey", up="darkgoldenrod1", down="steelblue")

    ## Plots
    degs[[i]][[j]]$plot_scatter = ggplot(tmp %>% dplyr::arrange(col, gene), aes(x=x, y=y, col=col)) + 
      ...
      geom_point() + 
      xlim(lims) + ylim(lims) +
      AddStyle(xlab=degs[[i]][[j]]$condition_group1, ylab=degs[[i]][[j]]$condition_group2, 
               col=cols, legend_position="bottom", legend_title="Filtered genes")

    # Feature plot of top 5 genes, sorted by the p-value
...
```{r deg_print_summary_to_html, results="asis", fig.show="asis"}
knitr_header_string = '
...
# Add additional colour lists (orig.ident, seurat_clusters and Phase were already added)
sc = ScAddLists(sc, lists=list(two_colour_gradient1=c(low=c("lightgrey", high = param$col)), three_colour_gradient1=c(low="steelblue", mid="lightgrey", high="darkgoldenrod1"), up_down=c(down="steelblue", up="darkgoldenrod1")), lists_slot="colour_lists")


# Add experiment details
Seurat::Misc(sc, "experiment") = list(
  experiment_name = param$project_id,
  organism = 'hg',
...
saveRDS(sc, file.path(param$path_out, "sc.rds"))
output_files[["sc_seurat"]] = file.path(param$path_out, "sc.rds")

#
# 2. Counts/metadata/annotation
#

# Counts as sparse matrix plus metadata
output_files[["assay"]] = ExportSeuratAssayData(sc, 
                      dir=file.path(param$path_out, "assay"), 
                      assays="RNA", 
                      slot="counts",
                      include_cell_metadata_cols=colnames(sc[[]]),
                      metadata_prefix=paste0(param$project_id, "."))

# Metadata as excel file
openxlsx::write.xlsx(x=sc[[]], file=file.path(param$path_out, "cell_metadata.xlsx"), row.names=TRUE, col.names=TRUE)
output_files[["cell_metadata"]] = file.path(param$path_out, "cell_metadata.xlsx")
...
openxlsx::write.xlsx(x=data.frame(seurat_id=rownames(sc), ensembl_gene_id=seurat_rowname_to_ensembl[rownames(sc)]) %>%
                     ...
                     file=file.path(param$path_out, "gene_annotation.xlsx"), 
                     row.names=FALSE, col.names=TRUE)
output_files[["gene_annotation"]] = file.path(param$path_out, "gene_annotation.xlsx")


#
# 3. Counts for 500 cells plus metadata
#
...
* Marker genes  
  + markers_cluster_vs_rest.xlsx: Excel file with one tab per cell cluster  
* Functional enrichment of marker genes per cell cluster  
  + Functions_marker_down_cluster_1.xlsx, Functions_marker_up_cluster_1.xlsx, ...: Excel files with one tab per database
* Differentially expressed genes
  + degs_contrast_row_1_results.xlsx, degs_contrast_row_2_results.xlsx, ...: Excel files per test
* Functional enrichment of differentially expressed genes 
  + degs_contrast_row_1_functions.xlsx, degs_contrast_row_2_functions.xlsx, ...: Excel files per test
...
write.table(loupe_umap, file=paste0(param$path_out, "/Loupe_umap.csv"), col.names=TRUE, row.names=FALSE, quote=FALSE, sep=",")
...
write.table(x=loupe_meta, file=paste0(param$path_out, "/Loupe_metadata.csv"), col.names=TRUE, row.names=FALSE, quote=FALSE, sep=",")
...
write.table(loupe_genesets, file=paste0(param$path_out, "/Loupe_genesets.csv"), col.names=TRUE, row.names=FALSE, quote=FALSE, sep=",")
```

* list files here
* shortly explain how to
...
```{r cellxgene_export, include=FALSE}
# Convert Seurat single cell object to python anndata object which will be accessible via reticulate here
adata = sceasy::convertFormat(sc, from="seurat", to="anndata", outFile=NULL)

# Set up correct colours (see https://chanzuckerberg.github.io/cellxgene/posts/prepare) to that colours match
adata$uns = dict(list(seurat_clusters_colors = unname(Misc(sc, "colour_lists")$seurat_clusters),
                           orig.ident_colors = unname(Misc(sc, "colour_lists")$orig.ident)))

adata$write(file.path(param$path_out, "sc.h5ad"))
output_files[["sc_anndata"]] = file.path(param$path_out, "sc.h5ad")

```

* list files here
* shortly explain howto
...
```{r cerebro_integration, message=FALSE, warning=FALSE, eval=F}
# Export to cerebro (almost functional)
