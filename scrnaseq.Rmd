...
* from "Systematic comparative analysis of single cell RNA-sequencing methods" `r knitcitations::citet("10.1186/s13059-019-1863-4")`
...
sc = purrr::map(sc, Seurat::CellCycleScoring, s.features=genes_s, g2m.features=genes_g2m, set.ident=FALSE)
...
plist = purrr::map(list_names(sc), function(n) {
  ...
  p = Seurat::VariableFeaturePlot(sc[[n]], assay="RNA", selection.method="vst") + ggtitle(n)
  p = PlotMystyle(p, col=c("grey", param$col))
...
plist = purrr::map(list_names(sc), function(n) {
  ...
  p = Seurat::VariableFeaturePlot(sc[[n]], assay="SCT", selection.method="sct") + ggtitle(n)
  p = PlotMystyle(p, col=c("grey", param$col))
  p = LabelPoints(plot=p, points=top10, repel=TRUE)
  p = p + theme(legend.position=c(0.2, 0.8),legend.background=element_rect(fill=alpha("white", 0.0)))
  return(p)
...
p = Seurat::VizDimLoadings(sc, dims=1:2, reduction="pca", combine=FALSE)
for (i in 1:length(p)) p[[i]] = PlotMystyle(p[[i]], col=param$col)
