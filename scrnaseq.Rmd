...
sc = purrr::map(sc, Seurat::NormalizeData, normalization.method = "LogNormalize", scale.factor = 10000)
...
sc = purrr::map(sc, Seurat::FindVariableFeatures, selection.method = "vst", nfeatures = 3000)
...
sc = purrr::map(sc, function(s) { Seurat::ScaleData(s, features=VariableFeatures(s)) })
```
...
# Scale RNA assay
sc = purrr::map(sc, function(s) { Seurat::ScaleData(s, features=VariableFeatures(s), vars.to.regress=param$vars_to_regress) })
...
dataset_names = names(sc)
sc = purrr::map(names(sc), function(n) { SCTransform(sc[[n]], vars.to.regress=param$vars_to_regress, min_cells=param$features_filter[[n]][["min_cells"]]) })
names(sc) = dataset_names
...
sc_feature_metadata = purrr::map(sc, function(s) {
  purrr::map(Assays(s), function(a) { return(s[[a]][[]]) })
})
...
```{r single_dataset_integration, eval=length(sc)>1 & param$integrate_samples[["method"]]=="merge"}
# Called when there is only one dataset and no integration needed
if (length(sc)>1 & param$integrate_samples[["method"]]=="merge") {
  # Feature metadata is removed by Seurat merge entirely; save separately and add again afterwards
  sc_feature_metadata = purrr::map(sc, )
...
```{r single_dataset_integration, eval=length(sc)==1, include=length(sc)==1}
# Called when there there are multiple datasets but no integration is needed (e.g. samples were multiplexed on the same chip)
if (length(sc)==1) {
  # Feature metadata is removed by Seurat merge entirely; need to add again afterwards
  sc_feature_metadata = purrr::map(sc, )
  sc1 = sc[[1]]
}
```
