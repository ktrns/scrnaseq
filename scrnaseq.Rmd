...
sc_cells_to_exclude  = purrr::map(list_names(sc), function(n) { 
  ...
  filter_result[["samples_to_drop"]] = names(cells_orig_sample[cells_orig_sample %in% param$samples_to_drop])
...
fig_height_vf = 3.5 * ceiling(length(names(sc))/2)
```
...
if (param$integrate_samples[["method"]]=="merge") {
  ...
  # Rerun SCTransform
  DefaultAssay(sc) = "SCT"
  sc = suppressWarnings(SCTransform(sc, vars.to.regress=param$vars_to_regress, min_cells=1, verbose=FALSE))
