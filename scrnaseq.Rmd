...
param$integrate_samples = list(method="standard", reference_dataset=1)
...
for (i in seq(nrow(datasets))) {
  ...
  if (type == "10x") {
    ...
    if (nrow(plate_information) > 0) {
      ...
      # Also set orig.ident column and identity
      sc[[name]][["orig.ident"]] = sc[[name]][["SampleName", drop=TRUE]]
      sc[[name]] = Seurat::SetIdent(sc[[name]], value="orig.ident")
    }
  } 
}
...
# In this case, just prepend the dataset name, e.g. set1.NC, set2.NC, ...
...
sc = purrr::map(names(sc), function(n) {
  if (any(sc[[n]][["orig.ident", drop=TRUE]] %in% duplicate_sample_names)) {
    ...
    sc[[n]][["orig.ident"]] = ifelse(sc[[n]][["orig.ident", drop=TRUE]] %in% duplicate_sample_names, 
                                   paste(n,sc[[n]][["orig.ident", drop=TRUE]] ,sep="."),
                                   sc[[n]][["orig.ident", drop=TRUE]])
...
sc_filtered_cells = purrr::map(names(sc), function(n) {
  ...
  # Samples to drop
  cell_identities = sc[[n]][["orig.ident", drop=TRUE]] # cells and sample names made unique
  cell_identities_old = sc[[n]][["orig.ident.old", drop=TRUE]]# cells and sample names as original
  filter_result[["sample_to_drop"]] = names(cell_identities[cell_identities %in% param$samples_to_drop | cell_identities_old %in% param$samples_to_drop])
 return(filter_result)
})
names(sc_filtered_cells) = names(sc)

# TODO We could add a summary here #
...
# Also drop the respective colours
sample_names = purrr::flatten_chr(purrr::map(sc, function(s){ unique(as.character(s[[]][["orig.ident"]])) }))
...
sc = purrr::map(sc, function(s) { Seurat::ScaleData(s, features=VariableFeatures(s), verbose=FALSE) })
```
...
dataset_names = names(sc)
sc = purrr::map(names(sc), function(n) { SCTransform(sc[[n]], vars.to.regress=param$vars_to_regress, min_cells=param$features_filter[[n]][["min_cells"]], verbose=FALSE) })
names(sc) = dataset_names
...
# Show variable genes
plist = purrr::map(names(sc), function(n) {
...
cell_subset_ids = purrr::map(sc, function(s) {
  ...
  cell_subset = sample(cells, min(100, length(cells)))
...
```{r dataset_integration_preparation}
# Save actual method in variable for chunks
if(length(sc)==1) param$integrate_samples[["method"]] = "single"
integrate_method = param$integrate_samples[["method"]]

# Feature metadata is removed by Seurat merge entirely; save separately for each assay and add again afterwards (for now only feature_id, feature_name and feature_type)
sc_feature_metadata = purrr::map(sc, function(s) {
  s = sc[[1]]
  a = "SCT"
  s[[a]][[c("feature_id","feature_name","feature_type")]]
  purrr::map(Assays(s), function(a) { return(s[[a]][[c("feature_id","feature_name","feature_type")]]) })
})
...
```{r single_dataset_integration, eval=integrate_method=="single", include=integrate_method=="single", echo=integrate_method=="single"}
# Called when there there are multiple datasets but no integration is needed (e.g. samples were multiplexed on the same chip)
if (integrate_method=="single") {
  sc1 = sc[[1]]
}
```


```{r multi_dataset_integration_merge, eval=integrate_method=="merge", include=integrate_method=="merge", echo=integrate_method=="merge"}
# Called when there is only one dataset and no integration needed
if (integrate_method=="merge") {
   sc1 = merge(x=sc[[1]], y=sc[2:length(sc)], project=param$project)
}
```
...
```{r choose_norm_method}
#sc = purrr::map()
```


```{r multi_dataset_integration, eval=length(sc)>1, include=length(sc)>1}

# standard workflow (RNA)
sc = purrr::map(sc, function(s) {DefaultAssay(s) = "RNA"; return(s)})
sc_anchors =  Seurat::FindIntegrationAnchors(object.list=sc, dims=1:30, normalization.method="LogNormalize")
sc1 = IntegrateData(anchorset=sc_anchors, dims=1:30)
