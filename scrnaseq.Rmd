...
param$features_filter = list(min_counts=1, min_cells=3) # feature has to be found by at least one count in one cell
...
sc_cells_to_exclude  = purrr::map(list_names(sc), function(n) { # ASK ANDREAS
  filter_result = purrr::map(list_names(param$cell_filter[[n]]), function(f) {
    ...
    } else if (is.character(filter)) { # ASK ANDREAS
...
knitr::kable(sc_cells_to_exclude_summary, align="l", caption="Summary excluded cells") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>% 
...
# Summarise
sc_features_to_exclude_summary = as.data.frame(purrr::map(sc_features_to_exclude, length))
rownames(sc_features_to_exclude_summary) = c("Genes")
knitr::kable(sc_features_to_exclude_summary, align="l", caption="Summary excluded features") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>% 
  kableExtra::scroll_box(width="100%")
...
We next need to decide how many PCs we want to use for analyses. The following "Elbow plot" is designed to help us make an informed decision. In this plot, PCs are ranked based on the percentage of variance they explain. 
