...
param$deg_contrasts = data.frame(condition_column=c("orig.ident", "Phase"),
                                 condition_group1=c("pbmc_10x", "G1"),
                                 condition_group2=c("PBMC1_Smart-seq2", "G2M"))
...
if (ncol(marker_degs)>0 & "avg_logFC" %in% colnames(marker_degs) & !"avg_log2FC" %in% colnames(marker_degs)) {
...
if (ncol(marker_degs) == 0){
  marker_degs$avg_log2FC = marker_degs$p_val = marker_degs$p_val_adj = marker_degs$pct.1 = marker_degs$pct.2 = marker_degs$cluster = as.numeric()
  marker_degs$gene = as.character()
}
...
if ((nrow(marker_degs) > 0) & (nrow(marker_degs_filt$all) > 0)) {
  
  # Add average data to degs table and write to file
  marker_degs_out = cbind(marker_degs_filt$all, DegsAvgData(sc, genes=marker_degs_filt$all$gene))
  DegsWriteToFile(marker_degs_out, annot_ensembl=annot_ensembl, file=paste0(param$path_out, "/marker_degs_cluster_vs_rest.xlsx"))

  # Plot number of differentially expressed genes
  p = DegsPlotNumbers(degs_up=marker_degs_filt$up, degs_down=marker_degs_filt$down, 
                      title=paste0("Number of DEGs, comparing each cluster to the rest\n(FC=", 2^param$marker_log2FC, ", adj. p-value=", param$marker_padj, ")")) 
  p
  
} else { 

  # Do not run the following marker_degs-related chunks
  marker_degs_found = FALSE
}
```

```{r no_marker_degs_found, eval=!marker_degs_found}
if (!marker_degs_found) warning("No differentially expressed genes found. The following related code is not executed, no degs-related plots and tables are generated.")
```
...
if (marker_degs_found) {
  ...
  p = Seurat::DoHeatmap(sc, features=marker_degs_filt$all$gene, group.colors=param$col_clusters, label=FALSE) + 
    NoLegend() + 
...
if (marker_degs_found) {
  ...
  p = Seurat::DoHeatmap(sc, features=marker_degs_filt$up$gene, group.colors=param$col_clusters, label=FALSE) + 
    NoLegend() + 
    theme(axis.text.y=element_blank()) +
...
if (marker_degs_found) {
  # Heatmap of all differentially expressed genes
  p = Seurat::DoHeatmap(sc, features=marker_degs_filt$down$gene, group.colors=param$col_clusters, label=FALSE) + 
    NoLegend() + 
    theme(axis.text.y=element_blank()) +
...
```{r marker_degs_functional_enrichment, results="hide"}
# Note: We need to run this chunk to initialize the variable "marker_degs_enriched", which is used during the Cerebro export
if (marker_degs_found) {
  # DEGs up and down per cluster
  marker_genesets_up = lapply(cluster_all, function(x) {
    tmp = marker_degs_filt$up %>% 
  ...
  marker_genesets_down = lapply(cluster_all, function(x) {
    tmp = marker_degs_filt$down %>% 
      dplyr::filter(cluster==x) %>% 
      dplyr::pull(gene)
    # Pick the first matching Entrez symbol
    tmp = sapply(tmp, function(x) seurat_rowname_to_entrez[[x]][1]) %>% 
  ...
  names(marker_genesets_up) = paste0("DEG_up_cluster_", cluster_all)
  names(marker_genesets_down) = paste0("DEG_down_cluster_", cluster_all)
  marker_genesets = c(marker_genesets_up, marker_genesets_down)
  
  # Loop through gene lists
  marker_degs_enriched = list()
  for (i in seq(marker_genesets)) {
    if (length(marker_genesets[[i]]) >= 3) {
      marker_degs_enriched[[i]] = enrichR::enrichr(marker_genesets[[i]], databases=param$enrichr_dbs)
      marker_degs_enriched[[i]] = purrr::map(marker_degs_enriched[[i]], function(df) {
        df %>% dplyr::filter(Adjusted.P.value < param$p_enrichr)
      })
    
      # No enrichments found -> remove entry from the results 
      idx_rm = sapply(seq(marker_degs_enriched[[i]]), function(j) nrow(marker_degs_enriched[[i]][[j]]) == 0)
      if (sum(idx_rm) == length(param$enrichr_dbs)) {
        message("Geneset ", names(marker_genesets)[i], " does not result in any functional enrichments.")
        marker_degs_enriched[[i]] = NA
      } else if (sum(idx_rm) > 0) {
        marker_degs_enriched[[i]][idx_rm] = NULL
      }
    } else { 
      message("Geneset ", names(marker_genesets)[i], " has less than 3 genes, we skip enrichr.")
      marker_degs_enriched[[i]] = NA
    }
  }
  names(marker_degs_enriched) = names(marker_genesets)

  # Remove NA entries for lists that were too short to be tested
  is_na = is.na(marker_degs_enriched)
  if (sum(is_na) > 0) marker_degs_enriched = marker_degs_enriched[!is_na]

  # Write enrichment results to file
  marker_degs_enriched_top = matrix(NA, nrow=0, ncol=6)
  colnames(marker_degs_enriched_top) = c("GeneSet", "Database", "Term", "Overlap", "Adjusted_pval", "Genes")
  for (i in seq(marker_degs_enriched)) { 
    if (!is.null(marker_degs_enriched[[i]])) { 
      openxlsx::write.xlsx(marker_degs_enriched[[i]], file=paste0(param$path_out, "/Functions_", names(marker_degs_enriched)[i], ".xlsx"))
    }
  }
} else { 
  marker_degs_enriched=list()
...
if (marker_degs_found) {
  for (i in seq(marker_degs_enriched)) { 
    if (!is.null(marker_degs_enriched[[i]])) { 
      ...
      for (j in seq(marker_degs_enriched[[i]])) {
          marker_degs_enriched_top = rbind(marker_degs_enriched_top, 
                               c(names(marker_degs_enriched)[i], 
                               names(marker_degs_enriched[[i]])[j], 
                               marker_degs_enriched[[i]][[j]][1, c("Term", "Overlap", "Adjusted.P.value", "Genes")]))
      }
    }
  }

  # Print table of top terms per gene set
  knitr::kable(marker_degs_enriched_top, align="l", caption="Top enriched term per geneset") %>% 
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>% 
    kableExtra::scroll_box(width="100%", height="700px")
}
```
...
```{r computing_deg_contrasts, warning=FALSE}
...
deg_contrasts_list = SetupDegContrastsList(sc, param$deg_contrasts, param$latent_vars)

# Add the actual data to the list
deg_contrasts_list = purrr::map(deg_contrasts_list, function(contrast){
  # cells indices
  ...
  contrast[["cells_group1_idx"]] = match(colnames(sc)[cells_group1_idx], colnames(contrast[["object"]]))
  contrast[["cells_group2_idx"]] = match(colnames(sc)[cells_group2_idx], colnames(contrast[["object"]]))
  
  return(contrast)
})

# Compute the tests; TODO: this chunk may be done in parallel in future
degs = purrr::map(deg_contrasts_list, function(contrast) {
  # Note: a contrast is invalid if there is an error message
  if (length(contrast$error_messages)==0) {
    test_results = DegTestCellSets(object=contrast[["object"]],
                                   slot=contrast[["slot"]],
                                   cells_1=colnames(contrast[["object"]])[contrast[["cells_group1_idx"]]],
                                   cells_2=colnames(contrast[["object"]])[contrast[["cells_group2_idx"]]],
