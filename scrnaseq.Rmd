...
* from "Systematic comparative analysis of single cell RNA-sequencing methods" (Ding et al 2018)
...
param$normalisation_default = "RNA"
...
knitr::kable(head(sc[[1]][[]], 5), align="l", caption="Cell meta-data, top 5 rows") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"))
...
for (i in names(cell_qc_features)) {
  ...
  p_list[[i]] = PlotMystyle(p_list[[i]], title=i, legend_position="none") + xlab("")
  ...
  if (nrow(cell_filter_for_plot)>0) {
    p_list[[i]] = p_list[[i]] + geom_segment(data=cell_filter_for_plot,aes(x=as.integer(as.factor(orig.ident))-0.5, xend=as.integer(as.factor(orig.ident))+0.5, y=value, yend=value),lty=2, col="orange")
  }
}
...
# Summarise
sc_excluded_cells_summary = purrr::map_dfr(sc_cells_to_exclude ,function (s) {
  return(as.data.frame(purrr::map(s,length))) 
  })
rownames(sc_excluded_cells_summary) = names(sc_cells_to_exclude )
knitr::kable(sc_excluded_cells_summary, align="l", caption="Cell filtering summary") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>% 
...
Experience shows that the 1,000 - 2,000 genes with the highest cell-to-cell variation are often enough to elucidate the global structure of a single cell dataset. For example, cell type-specific genes should highly variable across cells while house keeping genes should be similarly expressed across cells. In addition, such genes should be sufficiently expressed since expression level is strongly positively correlated with technical drop out, i.e., failing to measure the gene in a cell.

Here, we use the variance-stabilizing transformation (vst) method implemented in Seurat (`r knitcitations::citet("10.1186/s13059-019-1874-1")`. This method first models the technical heterogeneity as relationship between mean and variance using local polynomial regression. The model is then used to calculate the expected variance based on the observed mean. The residual variance (the difference between observed and expected variance) then likely reflects biological heterogeneity. The top 3000 genes are used for further analysis.
...
cat("\n\n#### Advanced normalisation (SCT)\n\n")
  ...
  cell_subset_norm2 = GetAssayData(subset(sc, cells=purrr::flatten_chr(cell_subset_ids), features=shared_sct_features), assay="SCTintegrated", slot="data")
...
```{r dimensionality}
if (DefaultAssay(sc)!="SCT" & DefaultAssay(sc)!="SCTintegrated") {
  ...
  PlotMystyle(p, title="Jack Straw plot", legend_position="bottom")
}
...
p = Seurat::DimPlot(sc, reduction="umap", label=TRUE) + scale_colour_manual("Cluster", labels=cluster_labels, values=param$col_clusters)
...
p = Seurat::FeaturePlot(sc, features="S.Score", pt.size=1, min.cutoff="q1", max.cutoff="q99", cols=c("lightgrey", param$col), label=TRUE)
...
p = Seurat::RidgePlot(sc, features=genes_example, slot="counts", combine=FALSE, assay="RNA", cols=param$col_clusters)
names(p) = genes_example
...
p = Seurat::RidgePlot(sc, features=genes_example, combine=FALSE, assay="RNA", cols=param$col_clusters)
names(p) = genes_example
for (i in names(p)) p[[i]] = PlotMystyle(p[[i]], title=i, legend_title="Cell identity")
p = patchwork::wrap_plots(p, ncol=2) + 
