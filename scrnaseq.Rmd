...
params:
  ...
  # Downsample data to at most n cells per sample
  #   NULL to deactivate
  ...
  # General parameters #
  ######################
  # Main colour to use for plots (Any valid R color or RGB hexadecimal code)
  col: "palevioletred"
...
sc = Seurat::FindClusters(sc, resolution=cluster_resolutions, algorithm=4, verbose=FALSE)
...
for(r in cluster_resolutions) {
  sc = BuildPhylogeneticTree = function(sc, category=paste0(DefaultAssay(sc), "_snn_res.", r), pc_n=pc_n, reorder=FALSE)
}
...
sc$seurat_clusters_ordered = sc$seurat_clusters
...
if (markers_found) {
  ...
  fig_height_5cols = height_per_row * nr_rows_5cols %>% min(100)
...
if (markers_found) {
  ...
  p = Seurat::DoHeatmap(sc, group.by="seurat_clusters_ordered", features=markers_filt$all$gene, group.colors=param$col_clusters, label=FALSE, cells=cells_subset) +
...
if (markers_found) {
  ...
  p = Seurat::DoHeatmap(sc, group.by="seurat_clusters_ordered", features=features_subset, group.colors=param$col_clusters, label=FALSE, cells=cells_subset) + 
...
openxlsx::write.xlsx(x=data.frame(seurat_id=rownames(sc)) %>%
                       dplyr::filter(seurat_id %in% names(seurat_rowname_to_ensembl)) %>%
                       dplyr::mutate(ensembl_gene_id=seurat_rowname_to_ensembl[seurat_id]) %>%
