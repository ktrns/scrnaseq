...
path_to_git = "~/scrnaseq"
source(paste0(path_to_git, "/R/functions_io.R"))
source(paste0(path_to_git, "/R/functions_plotting.R"))
source(paste0(path_to_git, "/R/functions_analysis.R"))
source(paste0(path_to_git, "/R/functions_util.R"))
...
The following table shows metadata (columns) of the first 5 cells (rows). These metadata provide additional information about the cells in the dataset, such as the sample a cell belongs to ("orig.ident"), or the above mentioned number of unique genes detected ("nFeature"). 
```{r qc}
...
# Metadata 
knitr::kable(head(sc@meta.data, 5), align="l", caption="Metadata, top 5 rows") %>% 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>% 
...
message(fig_height_knownMarkers_avgplot)
if (any(idx_avgplot)) {
  known_markers_avgplot = known_markers_list[idx_avgplot]
  message(length(known_markers_avgplot))
  ...
  p = Seurat::FeaturePlot(sc, features=names(known_markers_avgplot), cols=c("lightgrey", param$col), combine=FALSE)
...
```{r versions}
out = scrnaseq_session_info(path_to_git)
