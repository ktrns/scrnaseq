...
# Additional variables to be accounted for statistical tests
param$latent_vars = c()
...
# DEG contrasts
#
# The parameter takes a table in form of a data.frame or an Excel file.
...
# condition_group1: condition levels in group 1; can also be multiple levels concatenated by the plus character; empty string = all levels not in group2
# condition_group2: condition levels in group 2; separate multiple levels concatenated by the plus character; empty string = all levels not in group1
#
# These columns are optional:
# subset_column: categorial column in the cell metadata to subset before testing; specify "orig.ident" for sample and "seurat_clusters" for cluster (default: none)
# subset_group: subset levels; for the individual analysis of multiple levels separate by semicolons; for the joint analysis of multiple levels concatenate by the plus character (default: none)
# assay: Seurat assay to test on; can also be a Seurat dimensionality reduction (default: RNA)
# slot: when Seurat assay object, which slot to use (default: data)
...
# latent_vars: Additional variables to be accounted for. Multiple variables need to be concatenated by semicolons. Will overwrite the default by param$latent_vars (default: none).
param$deg_contrasts = data.frame(condition_column=c("orig.ident", "orig.ident", "Phase"),
                                 condition_group1=c("pbmc_10x", "pbmc_10x", "G1"),
                                 condition_group2=c("PBMC1_Smart-seq2", "PBMC1_Smart-seq2", "G2M"),
...
# Seurat-compatible unique rowname will now be used as rownames for annot_ensembl
rownames(annot_ensembl) = unname(ensembl_to_seurat_rowname[annot_ensembl[, ensembl]])
...
for (i in seq(nrow(datasets))) {
  ...
  if (type == "10x") {
    ...
    # Note: If a dataset contains multiple samples, the sample names could clash with sample names of other datasets. This will be taken care of next.
...
if (any(duplicated(purrr::flatten_chr(purrr::map(sc, colnames))))) {
...
# make annotation bullet-proof
#gene_names = unique(purrr::flatten_chr(purrr::map(sc, rownames)))
...
```{r filter_size_after, results="asis"}
sc
```
...
Note that removing all signal associated to cell cycle can negatively impact downstream analysis. For example, in differentiating processes, stem cells are quiescent and differentiated cells are proliferating (or vice versa), and removing all cell cycle effects can blur the distinction between these cells. As an alternative, we can remove the difference between G2M and S phase scores. This way, signals separating non-cycling and cycling cells will be maintained, while differences amongst proliferating cells will be removed. For a more detailed explanation, see the cell cycle vignette for Seurat `r Cite("https://satijalab.org/seurat/v3.1/cell_cycle_vignette.html")`. Cell cycle effect removed for this report: `r param$cc_remove`; _all_ cell cycle effects removed for this report: `r param$cc_remove_all`.   
...
if (param$integrate_samples[["method"]]!="single") {
  ...
  # When merging, cell metadata is merged but factors are not kept
  sc_cell_metadata = suppressWarnings(purrr::map_dfr(sc, function(s){ s[[]] }) %>% as.data.frame())
...
marker_degs = suppressMessages(Seurat::FindAllMarkers(sc, assay="RNA", test.use="MAST",
...
# If no markers were found, initialize the degs table so that further downstream (export) chunks run
if (ncol(marker_degs)==0) marker_degs = DegsEmptyMarkerResultsTable(levels(Idents(sc)))

# For Seurat versions until 3.2, log fold change is based on the natural log. Convert to log base 2.
if ("avg_logFC" %in% colnames(marker_degs) & !"avg_log2FC" %in% colnames(marker_degs)) {
  lfc_idx = grep("avg_log\\S*FC", colnames(marker_degs))
  marker_degs[,lfc_idx] = marker_deg_results[,lfc_idx] / log(2)
  col_nms = colnames(marker_degs)
  col_nms[2] = "avg_log2FC"
  colnames(marker_degs) = col_nms
}

# Sort markers
marker_degs = marker_degs %>% DegsSort(group=c("cluster"))
  
# Filter markers 
marker_degs_filt = DegsFilter(marker_degs, cut_log2FC=param$marker_log2FC, cut_padj=param$marker_padj)
marker_degs_found = nrow(marker_degs_filt$all)>0

# Add average data to table and write to file
marker_degs_out = cbind(marker_degs_filt$all, DegsAvgDataPerIdentity(sc, genes=marker_degs_filt$all$gene))
marker_degs_results_file = DegsWriteMarkerToFile(marker_degs_out, annot_ensembl=annot_ensembl, file=paste0(param$path_out, "/marker_degs_cluster_vs_rest.xlsx"))


# Plot number of differentially expressed genes
p = DegsPlotNumbers(marker_degs_filt$all, 
                      group="cluster", 
                      title=paste0("Number of DEGs, comparing each cluster to the rest\n(FC=", 2^param$marker_log2FC, ", adj. p-value=", param$marker_padj, ")")) 


if (marker_degs_found) {
  print(p)
} else {
...
```{r degs_table, eval=marker_degs_found}
if (marker_degs_found) {
  marker_degs_top = DegsUpDisplayTop(marker_degs_filt$up, n=5)

  # Show table
  knitr::kable(marker_degs_top, align="l", caption="Up to top 5 marker genes per cell cluster") %>% 
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>% 
...
```{r marker_degs_plot, eval=marker_degs_found}
if (marker_degs_found) {
  # Get top 1 gene per cluster and plot
  if (nrow(marker_degs_filt$all) > 0) {
    marker_degs_examples = marker_degs_filt$up %>% 
      dplyr::group_by(cluster) %>% 
      dplyr::top_n(n=1, wt=avg_log2FC) %>% 
      dplyr::select(cluster, gene) %>% 
      as.data.frame()
  } else { 
    marker_degs_examples = data.frame(cluster=numeric(), gene=character())
  }
  marker_degs_examples = setNames(marker_degs_examples$gene, paste0(marker_degs_examples$cluster, ": ", marker_degs_examples$gene))
}
```

```{r marker_degs_fig_heights}
# Note: We need to run this chunk as it specifies a variable that is used in chunk definitions below
if (marker_degs_found) {
  # Each row contains 2 plots
  nr_rows = ceiling(length(marker_degs_examples)/2)

  # The height of each plot might depend on the number of clusters 
  height_per_row = max(2, 0.3 * length(unique(Idents(sc))))

  # Total height of plots 
  fig_height_marker_degs = max(5, height_per_row * nr_rows)
} else {
  fig_height_marker_degs = 7
}
```

#### Feature plots
```{r marker_degs_plot_umap, eval=marker_degs_found, fig.height=fig_height_marker_degs}
if (marker_degs_found) {
  # Shows gene expression on the UMAP
  p_list = Seurat::FeaturePlot(sc, features=marker_degs_examples, cols=c("lightgrey", param$col), combine=FALSE, label=TRUE)
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle(title=names(marker_degs_examples)[i])
  p = patchwork::wrap_plots(p_list, ncol=2) + 
...
#### Violin plots (raw)
```{r marker_degs_plot_violin_raw, eval=marker_degs_found, fig.height=fig_height_marker_degs}
if (marker_degs_found) {
  # Violin plot of raw gene expression counts
  p_list = Seurat::VlnPlot(sc, features=marker_degs_examples, assay="RNA", slot="counts", combine=FALSE, pt.size=0.2)
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle(title=names(marker_degs_examples)[i], 
...
```{r marker_degs_plot_violin_norm, eval=marker_degs_found, fig.height=fig_height_marker_degs}
if (marker_degs_found) {
  # Violin plot of normalised gene expression data
  p_list = Seurat::VlnPlot(sc, features=marker_degs_examples, combine=FALSE, pt.size=0.2)
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle(title=names(marker_degs_examples)[i], 
                                                              legend_title="Cluster", 
                                                              fill=param$col_clusters, 
                                                              xlab="Cluster") 
...
#### Ridge plots (raw)
```{r marker_degs_plot_ridge_raw, eval=marker_degs_found, fig.height=fig_height_marker_degs, message=FALSE}
if (marker_degs_found) {
  # Ridge plot of raw gene expression counts
  p_list = Seurat::RidgePlot(sc, features=marker_degs_examples, assay="RNA", slot="counts", combine=FALSE)
  for (i in seq(p_list)) {
    p_list[[i]] = p_list[[i]] + AddStyle(title=names(marker_degs_examples)[i], 
                                         legend_title="Cluster", 
                                         fill=param$col_clusters, 
                                         ylab="Cluster")
  }
...
```{r marker_degs_plot_ridge_norm, eval=marker_degs_found, fig.height=fig_height_marker_degs, message=FALSE}
if (marker_degs_found) {
  # Ridge plot of normalised gene expression data
  p_list = Seurat::RidgePlot(sc, features=marker_degs_examples, slot="data", combine=FALSE)
  for (i in seq(p_list)) {
    p_list[[i]] = p_list[[i]] + AddStyle(title=names(marker_degs_examples)[i], 
                                         legend_title="Cluster", 
                                         fill=param$col_clusters, 
...
#### Dot plot
```{r marker_degs_plot_dot, eval=marker_degs_found, fig.height=fig_height_marker_degs*2}
if (marker_degs_found) {
  # Visualises how feature expression changes across different clusters
  p_list = lapply(levels(Idents(sc)), function(cl) {
    genes = marker_degs_top %>% dplyr::filter(cluster==cl) %>% dplyr::pull(gene)
    if (length(genes) > 0) { 
...
```{r marker_degs_heatmap_all, eval=marker_degs_found, fig.height=20}
if (marker_degs_found) {
  ...
  features_per_group = 300/length(levels(marker_degs_filt$all$cluster))
  features_subset = marker_degs_filt$all %>% 
    dplyr::group_by(cluster) %>% 
    dplyr::top_n(n=features_per_group, wt=abs(avg_log2FC)) %>% 
  ...
  p = Seurat::DoHeatmap(sc, features=features_subset, group.colors=param$col_clusters, label=FALSE, cells=cells_subset) + 
    ...
    ggtitle("Heatmap of scaled gene expression data, top genes differentially expressed between a cluster and the rest")
  p
...
```{r marker_degs_heatmap_all_up, eval=marker_degs_found, fig.height=20}
if (marker_degs_found) {
  # This will sample 500 cells; the number of cells per seurat_cluster will be proportional
  cells_subset = ScSampleCells(sc, n=500, group="seurat_clusters", group_proportional=TRUE)
  # With fig.height = 20, 300 features can be shown; distribute among clusters
  features_per_group = 300/length(levels(marker_degs_filt$up$cluster))
  features_subset = marker_degs_filt$up %>% 
    dplyr::group_by(cluster) %>% 
...
```{r marker_degs_heatmap_all_down, eval=marker_degs_found, fig.height=20}
if (marker_degs_found) {
  # This will sample 500 cells; the number of cells per seurat_cluster will be proportional
  cells_subset = ScSampleCells(sc, n=500, group="seurat_clusters", group_proportional=TRUE)
  # With fig.height = 20, 300 features can be shown; distribute among clusters
  features_per_group = 150/length(levels(marker_degs_filt$down$cluster))
  features_subset = marker_degs_filt$down %>% 
    dplyr::group_by(cluster) %>% 
    dplyr::top_n(n=features_per_group, wt=-avg_log2FC) %>% 
    dplyr::arrange(cluster, avg_log2FC) %>%
    dplyr::pull(gene) %>%
  ...
  # Heatmap of all down-regulated genes
  p = Seurat::DoHeatmap(sc, features=features_subset, group.colors=param$col_clusters, label=FALSE, cells=cells_subset) + 
    NoLegend() + 
    theme(axis.text.y=element_text(size=8)) +
    ggtitle("Heatmap of scaled gene expression data, top genes down-regulated in a cluster compared to the rest")
  p
}
```
...
marker_degs_enriched=list()
```

```{r marker_degs_functional_enrichment, results="hide", eval=marker_degs_found}
if (marker_degs_found) {
  # Convert seurat names of upregulated marker per cluster to entrez; use named lists for translation
  marker_genesets_up = sapply(levels(Idents(sc)), function(x) {
    tmp = marker_degs_filt$up %>% dplyr::filter(cluster==x) %>% dplyr::pull(gene)
  ...
  marker_genesets_up_enriched = purrr::map(marker_genesets_up, EnrichrTest, databases=param$enrichr_dbs, padj=param$p_enrichr)
  ...
  marker_genesets_down = sapply(levels(Idents(sc)), function(x) {
    tmp = marker_degs_filt$down %>% dplyr::filter(cluster==x) %>% dplyr::pull(gene)
    tmp = sapply(tmp, function(x) seurat_rowname_to_entrez[[x]][1], USE.NAMES=TRUE, simplify=TRUE) %>% unlist() %>% as.character()
    return(tmp[!is.na(tmp)])
  }, USE.NAMES=TRUE, simplify=TRUE)
  
  #  Tests done by Enrichr
  marker_genesets_down_enriched = purrr::map(marker_genesets_down, EnrichrTest, databases=param$enrichr_dbs, padj=param$p_enrichr)
...
```{r functional_enrichment_results, eval=marker_degs_found, results="asis"}
...
if (marker_degs_found) {
  ...
  for(n in names(marker_genesets_top_enriched)){
    ...
    print(knitr::kable(marker_genesets_top_enriched[[n]][, c("Database", "Term", "Direction", "Adjusted.P.value", "Odds.Ratio")], align="l", caption="Top ten enriched terms per geneset") %>% 
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>% 
    kableExtra::scroll_box(width="100%", height="700px"))
...
degs_contrasts_list = purrr::map(degs_contrasts_list, function(contrast){
  ...
  # latent vars must be passed as data.frame
...
degs_contrasts_results = purrr::map(degs_contrasts_results, function(contrast) {
  ...
  enrichr_results_up = EnrichrTest(genes=degs_up, databases=param$enrichr_dbs, padj=param$p_enrichr)
  ...
  enrichr_results_down = EnrichrTest(genes=degs_down, databases=param$enrichr_dbs, padj=param$p_enrichr)
...
degs_result_files = purrr::map_chr(degs, function(degs_subsets) {
  ...
  file = DegsWriteToFile(degs_subsets_results, annot_ensembl=annot_ensembl, file=file)
...
for(i in seq(degs)) {
  ...
  print(
      knitr::kable(degs_subsets_results %>% 
                   ...
                   dplyr::summarise(Cells1=dplyr::first(cells1),Cells2=dplyr::first(cells2),
                                    DEGs=length(gene),DEGs_up=sum(avg_log2FC>0),DEGs_down=sum(avg_log2FC<0)),
                 align="l", caption="DEG summary", col.names = c("Subset", "Cells in group 1", "Cells in group 2", "# DEGs", "# DEGs upregulated", "# DEGs downregulated")) %>% 
      kableExtra::kable_styling(bootstrap_options=c("striped", "hover"))
    )
  cat('\n \n')
} 
...
# Export gene sets
loupe_genesets = data.frame(List=sapply(marker_degs_filt$up[,"cluster"], function(s) paste0("DEG_up_cluster_", s)), 
                            Name=marker_degs_filt$up[,"gene"], 
                            Ensembl=ifelse(marker_degs_filt$up[,"gene"] %in% names(seurat_rowname_to_ensembl),
                                           seurat_rowname_to_ensembl[marker_degs_filt$up[,"gene"]], marker_degs_filt$up[,"gene"]))
                            
loupe_genesets = rbind(loupe_genesets, 
                       data.frame(List=sapply(marker_degs_filt$down[,"cluster"], function(s) paste0("DEG_down_cluster_", s)), 
                                  Name=marker_degs_filt$down[,"gene"],
                                  Ensembl=ifelse(marker_degs_filt$down[,"gene"] %in% names(seurat_rowname_to_ensembl),
                                           seurat_rowname_to_ensembl[marker_degs_filt$down[,"gene"]], marker_degs_filt$down[,"gene"])))
...
# top expressed genes
sc = cerebroApp::getMostExpressedGenes(sc, groups=c("seurat_clusters","orig.ident"), assay=Seurat::DefaultAssay(sc))

gene_lists_for_cerebro = list()
if (marker_degs_found) {
  gene_lists_for_cerebro = split(marker_degs_filt$all$gene, marker_degs_filt$all$cluster)
