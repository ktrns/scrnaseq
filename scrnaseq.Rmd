...
param$path_data = data.frame(name=c("sample1", "sample2"),
                             type=c("10x", "10x"),
                             path=c("test_datasets/10x_pbmc_small/counts/sample1/", "test_datasets/10x_pbmc_small/counts/sample2/"),  
                             stats=c(NA, NA))
...
param$path_out = "test_datasets/10x_pbmc_small/results"
...
#   - k.score: How many neighbors for calculating scores (default: min(30, minimum number of cells in a sample))
...
param$deg_contrasts = data.frame(condition_column=c("orig.ident", "orig.ident"),
                                 condition_group1=c("sample1", "sample1"),
                                 condition_group2=c("sample2", "sample2"),
                                 subset_column=c("seurat_clusters", "seurat_clusters"),
                                 subset_group=c("1", "2"),
                                 test=c("MAST", "DESeq2"),
                                 padj=c(100,100))
...
```{r qc_plot_cells, fig.width=10, fig.asp=0.5, fig.retina=2}
...
p_list = c(p_list, p_list)

# p = 1
patchwork::wrap_plots(p_list[1], ncol=2) + patchwork::plot_annotation("Distribution of feature values") 

# p = 2
#patchwork::wrap_plots(p_list[1:2], ncol=2) + patchwork::plot_annotation("Distribution of feature values") 

# p = 3
#patchwork::wrap_plots(p_list[1:3], ncol=2) + patchwork::plot_annotation("Distribution of feature values") 
#p
...
if (any(!is.na(sc$Phase))) {
  ...
  message("No information about cell cycle available.")
}
```
...
if (markers_found) {
  ...
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle(title=names(markers_examples)[i], 
                                                              ...
                                                              xlab="Cluster") + theme(legend.position="none")
  p = patchwork::wrap_plots(p_list, ncol=2) + 
...
degs_contrasts_results = furrr::map_future(degs_contrasts_results, function(contrast) {
  # Get results table
...
openxlsx::write.xlsx(x=sc[[]], file=file.path(param$path_out, "data", "cell_metadata.xlsx"), row.names=TRUE, col.names=TRUE)
...
write.table(loupe_umap, file=file.path(param$path_out, "data", "Loupe_umap.csv"), col.names=TRUE, row.names=FALSE, quote=FALSE, sep=",")
...
write.table(x=loupe_meta, file=file.path(param$path_out, "data", "Loupe_metadata.csv"), col.names=TRUE, row.names=FALSE, quote=FALSE, sep=",")
...
adata$write(file.path(param$path_out, "data", "sc.h5ad"))
```
...
res = ExportToCerebro(sc=sc, 
                      path=file.path(param$path_out,"data","cerebro.crb"), 
