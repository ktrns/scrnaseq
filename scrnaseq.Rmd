...
# Where to store the annotation so that it has to be fetched only once
# if NULL, the annotation will be stored in output_directory/annotation
...
param$cc_genes_marker_file = file.path(param$path_out, "annotation", "cell_cycle_markers.xlsx")
...
error_messages = c(error_messages, check_ensembl(biomart="ensembl", 
                                                 ...
                                                 file_cc_markers=param$cc_genes_marker_file))
...
if (file.exists(param$cc_genes_marker_file)) {
  # Load from file
  genes_s = openxlsx::read.xlsx(param$cc_genes_marker_file, sheet=1)
  genes_g2m = openxlsx::read.xlsx(param$cc_genes_marker_file, sheet=2)
  ...
  # Write to file
  openxlsx::write.xlsx(list(S_phase=genes_s,G2M_phase=genes_g2m),file=param$cc_genes_marker_file)
}
...
```{r qc_plot_correlation, fig.height=6}
...
```{r highest_expression, fig.height=6}
...
```{r umap_by_cluster, fig.height=6}
...
```{r cells_per_cluster, fig.height=5}
...
```{r cellCycleEffect, fig.height=5}
...
```{r cellCycleEffect_umap_phases, fig.height=6}
...
```{r cellCycleEffect_umap_s, fig.height=6}
if (any(!is.na(sc$Phase))) {
...
```{r cellCycleEffect_umap_g2m, fig.height=6}
if (any(!is.na(sc$Phase))) {
...
```{r cellCycleEffect_umap_ccdiff, fig.height=6}
if (any(!is.na(sc$Phase))) {
...
## Number of counts 
```{r clusterQC_nCount_featurePlot, fig.height=4}
...
p1 = LabelClusters(p1, id="ident", box=FALSE, segment.color="black", fill="lightgrey")
...
## Number of features
```{r clusterQC_nFeature_featurePlot, fig.height=4}
...
```{r clusterQC_mt_featurePlot, fig.height=4}
...
```{r clusterQC_ribo_featurePlot, fig.height=4}
