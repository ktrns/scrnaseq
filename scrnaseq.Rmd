...
```{r todo}
# AP: 
# * finish the chunk sc_scafe
# * fix colours in the chunk cellxgene_export
# KS: 
# * add documentation on the output files -> list files and what to do with them
```

## Export to Loupe Cell Browser
If all provided datasets are of type "10x", we export the UMAP 2D visualisation, metadata such as the cell clusters, and lists of differentially expressed genes, so you can open and work with these in the Loupe Cell Browser.  
```{r loupe_export}
...
# Comment KS: 
#   Colours don't work yet
#   Colour-codes should not contain transparency, cellxgene cannot handle it 
#   The object with correct colour-codes can be loaded, but no colours are visible
...
# adata$uns = dict(list(seurat_clusters_colors = tolower(gsub(pattern="#..", x=unname(Misc(sc, "colour_lists")$seurat_clusters), replacement="#", perl=TRUE)),
#                            orig.ident_colors = tolower(gsub(pattern="#..", x=unname(Misc(sc, "colour_lists")$orig.ident), replacement="#", perl=TRUE))))
...
```{r save}
# Keep until AP fixed the next chunk
save.image(paste0(param$path_out, "/scrnaseq.RData"))
```

```{r sc_save, eval=FALSE}
...
* cellxgene files: 
  + sc.h5ad: Can be loaded into the cellxgene browser
* Cerebro files:
