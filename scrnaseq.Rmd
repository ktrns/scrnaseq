...
if (file.exists(param$file_annot)) {
  ...
  annot_mart = GetBiomaRt(biomart="ensembl", dataset=param$mart_dataset, mirror=mirror)
...
p1 = Seurat::FeaturePlot(sc, features="S.Score", pt.size=1, min.cutoff="q1", max.cutoff="q99", cols=c("lightgrey", param$col), label=T)
p1 = PlotMystyle(p1, title="UMAP, cells coloured by S phase")
p2 = Seurat::FeaturePlot(sc, features="G2M.Score", pt.size=1, min.cutoff="q1", max.cutoff="q99", cols=c("lightgrey", param$col), label=T)
p2 = PlotMystyle(p2, title="UMAP, cells coloured by G2M phase")
p = p1 + p2
p 
```

## Plot cells coloured by cell metadata
...
p = Seurat::FeaturePlot(sc, features=cell_qc_features, cols=c("lightgrey", param$col), combine=FALSE, label=T)
...
p = Seurat::DoHeatmap(sc, features=top$gene, group.colors=param$col_clusters) + NoLegend()
p
```
