...
param$mart.attributes = c("ensembl_gene_id", "external_gene_name", "hgnc_symbol", "entrezgene_accession", "chromosome_name", "start_position", "end_position", "percentage_gene_gc_content", "gene_biotype", "strand", "description")
...
ensembl_to_symbol = unique(annot.ensembl[,c("ensembl_gene_id","external_gene_name")])
ensembl_to_symbol = setNames(ensembl_to_symbol$external_gene_name,ensembl_to_symbol$ensembl_gene_id)

# gene symbol to ensembl id: named list to account with multiple genes having the same symbol
symbol_to_ensembl = unique(annot.ensembl[,c("ensembl_gene_id","external_gene_name")])
...
ensembl_to_seurat_rowname = unique(annot.ensembl[,c("ensembl_gene_id","external_gene_name")])
...
# gene symbol(s) to (seurat compatible unique) gene symbol: named list to account with multiple genes having the same symbol
symbol_to_seurat_rowname = purrr::map(symbol_to_ensembl,function(i){unname(ensembl_to_seurat_rowname[i])})
```
...
for(i in seq(nrow(datasets))){
  ...
  if(type == "10x"){
    ...
    sc[[name]] = Read10xDataset(path, project=name, feature_name_column=1, convert_feature_names=ensembl_to_seurat_rowname)
    
  
  } else if(type == "smartseq2"){
    # drop columns 2:6 and read into seurat object
    sc[[name]] = ReadSmartseq2Dataset(path, project=name, convert_feature_names=ensembl_to_seurat_rowname)
