...
try({invisible(knitcitations::citet("https://cran.r-project.org/web/packages/knitr/index.html"))})
```
...
sc = purrr::map(ListIndices(sc), function(i){
...
param$cell_filter = purrr::map(ListNames(sc), function(s) {
...
param$feature_filter = purrr::map(ListNames(sc), function(s) {
...
sc = purrr::map(ListNames(sc), function(n) {
...
cell_qc_features = ValuesToNames(cell_qc_features)
...
sc_cells_to_exclude  = purrr::map(ListNames(sc), function(n) { 
  filter_result = purrr::map(ListNames(param$cell_filter[[n]]), function(f) {
...
sc_features_to_exclude = purrr::map(ListNames(sc), function(n) {
...
sc = purrr::map(ListNames(sc), function(n) {
  ...
  features_to_keep = purrr::map(ValuesToNames(assay_names), function(a) {
...
p_list = purrr::map(ListNames(sc), function(n) {
...
if (param$integrate_samples[["method"]]!="single") {
  ...
  assay_names = unique(purrr::flatten_chr(purrr::map(ListNames(sc), function(n) { Seurat::Assays(sc[[n]]) } )))
  
  # Loop through all assays and accumulate meta data
  sc_feature_metadata = purrr::map(ValuesToNames(assay_names), function(a) {
    ...
    contains_neccessary_columns = purrr::map_lgl(ListNames(sc), function(n) { 
    ...
    other_feature_data = purrr::map(ListNames(sc), function(n) {
      df = sc[[n]][[a]][[]]
...
if (!is.null(param$file_known_markers)) {
  ...
  known_markers_list = lapply(colnames(known_markers), function(x) {
    ...
    m = !y %in% rownames(sc)
    if (any(m)){
      Warning(paste0("The following genes of marker list '", x, "' cannot be found in the data: ", FirstElementsToString(y[m], n=10)))
    }
    return(y[!m])
  })
...
if (markers_found) {
  ...
  # Join
  marker_genesets_top_enriched = purrr::map(ValuesToNames(names(marker_genesets_down_top_enriched)), function(n) {
...
* Differentially expressed genes (optional)
  + degs_contrast_row_1_results.xlsx, degs_contrast_row_2_results.xlsx, ...: Excel files per test
* Functional enrichment of differentially expressed genes (optional)
...
out = SessionInfo(param$path_to_git)
