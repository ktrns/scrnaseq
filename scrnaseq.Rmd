...
#raw counts plus metadata such that one can do a second analysis where one keeps only cells in cluster 1 (this could be done via the filter options)
#counts/data/z-scores for a representative set of cells (e.g. 500) as table together with metadata so that the user can plot their own heatmaps as #csv/excel
#cerebroApp/cellxgene/ucsc cell browser
#full metadata table as as csv/excel
#table with seurat name plus ensembl annotation
...
# Add technical
#Seurat::Misc(sc, "technical") = list(scrnaseq=git_repository_version(param$path_to_git), R=capture.output(devtools::session_info())
...
adata$write(paste0(param$path_out, "/sc.h5ad"))
```
