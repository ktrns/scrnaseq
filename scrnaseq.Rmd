...
knitr::opts_chunk$set(echo=TRUE,                     # output code
                      ...
                      fig.retina=2                   # DISCUSS: http://zevross.com/blog/2017/06/19/tips-and-tricks-for-working-with-images-and-figures-in-r-markdown-documents/#the-fig.retina-argument-is-a-resolution-multiplier
...
sample_names = purrr::flatten_chr(purrr::map(sc, function(s) { return(unique(as.character(s[[]][["orig.ident"]]))) }))
...
sc = purrr::map(list_names(sc), function(n) return(ScAddLists(sc[[n]], lists=list(samples=param$col_samples[n]), lists_slot="colour_lists")))
...
if (markers_found) {
  ...
  p_list = Seurat::VlnPlot(sc, features=markers_examples, assay="RNA", slot="counts", combine=FALSE, pt.size=0)
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle(title=names(markers_examples)[i], 
...
if (markers_found) {
  ...
  p_list = Seurat::VlnPlot(sc, features=markers_examples, combine=FALSE, pt.size=0)
  for (i in seq(p_list)) p_list[[i]] = p_list[[i]] + AddStyle(title=names(markers_examples)[i], 
                                                              legend_title="Cluster", 
                                                              fill=param$col_clusters, 
                                                              xlab="Cluster") 
...
if (markers_found) {
  ...
  p = patchwork::wrap_plots(p_list[!purrr::map_lgl(p_list, is.null)], ncol=2)
  print(p)
}
```
...
Seurat::Misc(sc, "technical") = list(scrnaseq_git=git_repository_version(param$path_to_git),
