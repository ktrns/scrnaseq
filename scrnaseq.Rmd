...
# library(MAST) # marker genes -> we load and unload it in the chunk
...
param$path.out = "~/testDatasets_10x_pbmc_1k_healthyDonor_v3Chemistry_Seurat/"
if(!file.exists(param$path.out)) dir.create(param$path.out, recursive=TRUE)

# Input data path in case Cell Ranger was run 
param$path.data = "testDatasets/10x_pbmc_1k_healthyDonor_v3Chemistry/filtered_feature_bc_matrix/"
param$file.mapping.stats = "testDatasets/10x_pbmc_1k_healthyDonor_v3Chemistry/pbmc_1k_v3_metrics_summary.csv"
...
annot.ensembl = read.delim(param$file.annot)
  ...
  if(is.null(param$file.annot)){
    param$file.annot = paste0(param$path.out, param$mart.dataset, '.annot.csv')
  }
