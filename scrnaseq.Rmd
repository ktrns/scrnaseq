...
1. Annotation:
- NULL: ensembl create full table (query all genes)
- use first column to do file annot
- 
...
# Input data path(s)
param$path.data = data.frame(name = "test", type = "smartseq2", path = c("../../../Desktop/work/Neuer Ordner/genecount/bfx1341.mm10.e98.txt"))
...
if(!is.null(param$file.annot) && file.exists(param$file.annot)){
  ...
  annot.ensembl = getBM(mart = annot.mart, attributes = mart.attributes)
...
ensembl.to.symbol = setNames(annot.ensembl$external_gene_name,annot.ensembl$ensembl_gene_id)
```
...
for(i in seq(nrow(datasets))){
  ...
  if(type == "10x"){
    ...
    sc[[name]] = read_10x_dataset(path,project=name)
    ...
    sc[[name]] = read_smartseq2_dataset(path,project=name,columns_to_drop = 2:6)
    ...
    if(nrow(plate_information)>0){
      ...
      sc[[name]] = AddMetaData(sc[[name]],plate_information)
      
      # also set identity
      sc[[name]] = SetIdent(sc[[name]],value="SampleName")
    }
  } 
}
