...
for (i in names(cell_qc_features)) {
  ...
  p_list[[i]] = PlotMystyle(p_list[[i]], title=i, legend_position="none", fill=param$col_samples) + xlab("") + theme(axis.text.x = element_text(angle=45, hjust=1))
...
```{r plot_variable_features_rna, warning=FALSE}
# Show variable genes
...
p = patchwork::wrap_plots(plist) + patchwork::plot_annotation("Variable genes")
p
```
...
```{r multi_dataset_integration_merge, eval=(param$integrate_samples[["method"]]=="merge"), include=(param$integrate_samples[["method"]]=="merge"), message=FALSE}
...
if (param$integrate_samples[["method"]]=="merge") {
  ...
  # Rerun SCTransform
  DefaultAssay(sc) = "SCT"
  sc = SCTransform(sc, vars.to.regress=param$vars_to_regress, min_cells=1, verbose=FALSE)
...
```{r plot_RLE_RNA_integrated, eval=(param$integrate_samples[["method"]]!="single" & param$integrate_samples[["method"]]!="merge"), include=(param$integrate_samples[["method"]]!="single" & param$integrate_samples[["method"]]!="merge"), results="asis"}
...
```{r plot_RLE_SCT_integrated, eval=(param$integrate_samples[["method"]]!="single" & param$integrate_samples[["method"]]!="merge"), include=(param$integrate_samples[["method"]]!="single" & param$integrate_samples[["method"]]!="merge"), results="asis"}
if (param$integrate_samples[["method"]]!="single" & param$integrate_samples[["method"]]!="merge") {
# Markdown text for this section (do not change intendation)
cat("\n\n#### Advanced normalisation (SCT)\n\n")
...
p = Seurat::DimHeatmap(sc, dims=1:1, cells=500, balanced=TRUE, fast=FALSE)
p
```

```{r pca_heatmaps, fig.height=20}
p = Seurat::DimHeatmap(sc, dims=1:20, cells=500, balanced=TRUE, fast=FALSE)
p
```
...
p = Seurat::DimPlot(sc, reduction="umap", label=TRUE) + scale_color_manual(values=param$col_clusters, labels=cluster_labels)
p = PlotMystyle(p, "UMAP, cells coloured by cluster identity", legend_position="bottom", legend_title="Clusters")
p
...
if (length(cell_samples) > 1) {
  ...
  p = PlotMystyle(p, "UMAP, cells coloured by sample of origin", legend_position="bottom")
  print(p)
}
```
...
p$data$seurat_clusters = sc[["seurat_clusters"]][rownames(p$data),]
p = LabelClusters(p, id = "seurat_clusters")
p = PlotMystyle(p, title="UMAP, cells coloured by cell cycle phases", legend_title="Phase")
p
```
...
```{r featurePlot_QC_mt}
p = Seurat::FeaturePlot(sc, features="percent_mt", cols=c("lightgrey", param$col), label=TRUE)
...
if(!is.null(param$file_known_markers)) {
  ...
  fig_height_knownMarkers_dotplot = fig_height_knownMarkers_avgplot = fig_height_knownMarkers_vect = 1
}
```
...
sc_markers$p_val_adj_score = -log2(sc_markers$p_val_adj) * sign(sc_markers$avg_logFC)
sc_markers = sc_markers %>% 
  dplyr::arrange(cluster, -p_val_adj_score, -avg_logFC) %>% 
  as.data.frame()
...
sc_markers_filt_top3 = sc_markers_filt %>% 
  ...
  dplyr::select(cluster, gene, avg_logFC, p_val, p_val_adj, pct.1, pct.2) %>% 
  as.data.frame()
...
sc_markers_filt_top3$avg_logFC = round(sc_markers_filt_top3$avg_logFC, digits=3)
...
fig_height_deg = max(5, 2*length(genes_example))
```
...
### Violin plots (raw)
```{r DEG_plot_violin_raw, fig.height=fig_height_deg, message=FALSE}
# Violin plot of raw gene expression counts
p = Seurat::VlnPlot(sc, features=genes_example, slot="counts", combine=FALSE, pt.size=0.2)
for (i in seq(p)) p[[i]] = PlotMystyle(p[[i]], title=genes_example_titles[i], legend_title="Cell identity", fill=param$col_clusters, xlab="Cluster")
p = patchwork::wrap_plots(p, ncol=2) + 
...
```{r DEG_plot_violin_norm, fig.height=fig_height_deg, message=FALSE}
# Violin plot of normalised gene expression data
p = Seurat::VlnPlot(sc, features=genes_example, combine=FALSE, pt.size=0.2)
for (i in seq(p)) p[[i]] = PlotMystyle(p[[i]], title=genes_example_titles[i], legend_title="Cell identity", fill=param$col_clusters, xlab="Cluster")
p = patchwork::wrap_plots(p, ncol=2) + 
...
### Ridge plots (raw)
```{r DEG_plot_ridge_raw, fig.height=fig_height_deg, message=FALSE}
...
```{r DEG_plot_ridge_norm, fig.height=fig_height_deg, message=FALSE}
...
We export the assay data, clustering, visualisation, marker genes and enriched pathways in a format that can be read by the Cerebro Browser (https://github.com/romanhaa/cerebroApp).  
...
res = ExportToCerebro(sc=sc, path=paste0(param$path_out,"/cerebro.crb"), param=param, project=param$project, species=cerebro_species, assay=DefaultAssay(sc), column_sample="orig.ident", column_cluster="seurat_clusters", column_ccphase="Phase", gene_lists=gene_lists_for_cerebro, marker_genes=sc_markers_filt, enriched_pathways=enriched)
```
