...
param$integrate_samples = list(method="merge", reference_dataset=1, dimensions=30)
...
if (param$integrate_samples[["method"]]=="standard") {
  ...
  run_standard_integration = function(sc_objs, ndims=30, vars_to_regress=c(), verbose=FALSE, assay="RNA") {
    ...
    if (assay == "RNA") {
      ...
      sc_objs = Seurat::ScaleData(sc_objs, 
                                  features=rownames(tmp_sc_objs[["RNAintegrated"]]), 
  ...
  # call function
  sc = run_standard_integration(sc, ndims=param$integrate_samples[["dimensions"]], vars_to_regress=param$vars_to_regress, assay=param$norm)
...
#### Feature plots
```{r degs_plot_umap, fig.height=fig_height_variable}
...
#### Violin plots (raw)
```{r degs_plot_violin_raw, fig.height=fig_height_variable}
...
```{r degs_plot_violin_norm, fig.height=fig_height_variable}
...
#### Ridge plots (raw)
```{r degs_plot_ridge_raw, fig.height=fig_height_variable, message=FALSE}
...
```{r degs_plot_ridge_norm, fig.height=fig_height_variable, message=FALSE}
...
#### Dot plot
```{r degs_plot_dot, fig.height=fig_height_variable*2}
...
```{r degs_heatmap_all, fig.height=20}
...
```{r degs_heatmap_all_up, fig.height=20}
...
```{r degs_heatmap_all_down, fig.height=20}
# Heatmap of all differentially expressed genes
...
# Export gene sets
loupe_genesets = data.frame(List=paste0("DEG_up_cluster_", degs_filt$up[,"cluster"]), 
                            Name=degs_filt$up[,"gene"], 
                            Ensembl=seurat_rowname_to_ensembl[degs_filt$up[,"gene"]])
loupe_genesets = rbind(loupe_genesets, 
                       data.frame(List=paste0("DEG_down_cluster_", degs_filt$down[,"cluster"]), 
