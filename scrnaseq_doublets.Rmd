...
if (params$norm == "LogNormalize") {
  ...
  # Scale data: the DoubletFinder tutorial uses only the variable genes but according to "Protocol for Benchmarking Computational Doublet-Detection Methods in Single-Cell RNA Sequencing" it might be better to scale all genes
  sc = Seurat::ScaleData(sc, features=rownames(sc[["RNA"]]), verbose=FALSE, vars.to.regress=c())
} else if (params$norm == "SCTransform") {
  
  # Run SCTransform: as for the standard normalisation strategy, we scale all data
  sc = SCTransform(sc, vars.to.regress=c(), min_cells=3, verbose=FALSE, return.only.var.genes=FALSE, method=ifelse(packages_installed("glmGamPoi"), "glmGamPoi", "poisson"))
}
```

# Dimensionality reduction, clustering and visualisation

Next, we use Principal Component Analysis (PCA) to overcoming noise and reducing the data to its essential components. Each principal component (PC) represents a "metafeature" that combines information across a correlated gene set. Then we also calculate a basic clustering of the data and use Uniform Manifold Approximation and Projection (UMAP) to visualise the dataset in 2D space. Note that here we use non-optimised standard algorithms. In a comprehensive single cell analysis, these algorithms are usually tuned to yield optimal results.
...
sc = Seurat::FindClusters(sc, resolution=0.5, algorithm=1, verbose=FALSE)
...
```{r pca_plots}
# Plot pca
p1 = Seurat::DimPlot(sc, reduction="pca", cols=sc_col_clusters, label=TRUE) + 
  AddStyle(title="Cells arranged by the first two PCs", legend_position="bottom", legend_title="Clusters")

p2 = Seurat::ElbowPlot(sc, ndims=min(20, ncol(sc))) + 
  geom_vline(xintercept=params$pc_n + .5, col="firebrick", lty=2) + 
  AddStyle(title="PCA Elbow plot") 
...
p = Seurat::DimPlot(sc, reduction="umap", label=TRUE) + 
  ...
  scale_color_manual(values=sc_col_clusters)
p
```

# Doublet detection

```{r doublet_detection, eval=F}
