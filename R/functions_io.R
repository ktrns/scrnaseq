...
FormatErrorMessage_glue = function(sep=", ", quote=TRUE, ...) {
  function(text, envir) {
    collapse = grepl("[*]$", text)
    if (collapse) {
      text = sub("[*]$", "", text)
...
ReadMetadata = function(file, ids=NULL) {
  ...
  assertthat::assert_that(extension %in% valid_extensions,
                          msg=glue::glue("Metadata file type must be: {valid_extensions*} (file can be gzipped).", .transformer=FormatErrorMessage_glue()))
...
ReadCounts_SmartSeq = function(path, type="RNA", transpose=FALSE) {.
  ...
  assertthat::assert_that(all(type %in% valid_types),
                          msg=glue::glue("'{type} must be: {valid_types*}.", .transformer=FormatErrorMessage_glue()))
...
ReadCounts_10x = function(path, type, transpose=FALSE) {
  ...
  if (length(counts_lst) == 1) {
    ...
    assertthat::assert_that(all(f),
                            msg=glue::glue("Dataset {path} does not contain the following types of data: {type[f]*} (named {converted_type[f]*} in the dataset).", .transformer=FormatErrorMessage_glue()))
