...
#' 
#' First column must contain the respective barcode or feature id.
#' 
#' @param h5ad_file Path to a character-separated file.
#' @return Metadata (data.frame format)
...
#' 
#' Reads only sheet 1. First column must contain the respective barcode or feature id.
#' 
...
#' 
#' @param rds_file Path to an R rds file.
#' @return Metadata (data.frame format)
ReadMetadata_excel = function(rds_file) {
  # Checks
...
#' @param feature_type_column If there is data for multiple feature types, which column (number) in the features file is used to identify the type. If there is only one type, set to NULL (default: NULL)
...
#' @return One sparse counts matrix per feature type (dgTMatrix format). Additional information on barcodes and features is attached as attributes barcode_data and feature_data.
ReadCounts_mtx = function(mtx_directory, mtx_file_name="matrix.mtx.gz", transpose=FALSE, barcodes_file_name="barcodes.tsv.gz", barcodes_column_names=FALSE, features_file_name="features.tsv.gz", features_column_names=FALSE, feature_type_column=NULL, delim="\t", strip_suffix=NULL) {
  ...
  counts_lst = purrr::map(feature_sets, function(f) {
    ...
    attr(cts, "barcode_data") = barcodes_data
    attr(cts, "feature_data") = features_data[f,, drop=FALSE]
    return(cts)
  })
...
#' 
#' @param h5ad_file Path to an anndata object in hdf5 format.
#' @return Sparse counts matrix (AnnDataMatrixH5 format). Additional information on barcodes and features is attached as attributes barcode_data and feature_data.
ReadCounts_h5ad = function(h5ad_file) {
  ...
  attr(counts_data, "barcode_data") = barcodes_data
  attr(counts_data, "feature_data") = features_data
...
ReadCounts_SmartSeq = function(path, transpose=FALSE) {.
  ...
  if (dir.exists(path)) {
    ...
    counts_data = ReadCounts_mtx(mtx_directory=path,
                   transpose=transpose,
                   ...
                   delim="\t",
                   strip_suffix=NULL)
    attr(counts_data, "barcode_data") = NULL
    attr(counts_data, "feature_data") = NULL
  } else {
    # character-separated file
    counts_data = ReadCounts_csv(csv_file=path, transpose=transpose)
  }
...
#' @return One sparse counts matrix per feature type (MatrixSubset format). Additional information on barcodes and features is attached as attributes barcode_data and feature_data.
ReadCounts_10x_hdf5 = function(hdf5_file) {
  ...
  counts_lst = purrr::map(feature_types, function(f) {
    ...
    attr(cts, "barcode_data") = barcodes_data
    attr(cts, "feature_data") = features_data[features_data$feature_type == f, , drop=FALSE]
...
#' @return One sparse counts matrix per feature type. Format is either MatrixSubset (when reading hdf5) or dgTMatrix (when reading from a matrix exchange format directory). Additional information on barcodes and features is attached as attributes barcode_data and feature_data.
ReadCounts_10x = function(path, transpose=FALSE) {
  # Checks
  assertthat::is.readable(path)
  
  if (dir.exists(path)) {
    # 10x market exchange format
    counts_data = ReadCounts_10x_mtx(mtx_directory=path)
  } else {
    # 10x hdf5 file
    counts_data = ReadCounts_10x_hdf5(hdf5_file=path)
  }
  return(counts_data)
}
...
#' @return One sparse counts matrix. Format is either AnnDataMatrixH5 (when reading anndata.h5ad) or dgTMatrix (when reading from a matrix exchange format directory). Additional information on barcodes and features is attached as attributes barcode_data and feature_data.
ReadCounts_ParseBio = function(path, transpose=FALSE) {
  # Checks
  assertthat::is.readable(path)
  
  if (dir.exists(path)) {
    # 10x market exchange format
    counts_data = ReadCounts_ParseBio_mtx(mtx_directory=path)
  } else {
    # 10x hdf5 file
    counts_data = ReadCounts_ParseBio_h5ad(h5ad_file=path)
  }
  return(counts_data)
}
...
#' @return  One sparse counts matrix per feature type (dgTMatrix format). Additional information on barcodes and features is attached as attributes barcode_data and feature_data
ReadCounts_ScaleBio = function(path, transpose=FALSE) {
  # Checks
  assertthat::is.readable(path)
