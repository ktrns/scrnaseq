...
Assays_10x = list("Gene Expression" = "RNA",
                  ...
                  "Custom" = "CUSTOM",
                  "Negative Control Codeword" = "NCC",
                  "Negative Control Probe" = "NCP",
                  "Unassigned Codeword" = "UC")
...
ReadCounts_SmartSeq = function(path, type="RNA", transpose=FALSE) {.
  ...
  assertthat::assert_that(all(type %in% Assays_Smartseq),
...
ReadCounts_10x = function(path, type, transpose=FALSE) {
  ...
  assay_to_featuretype = setNames(names(Assays_10x), Assays_10x)
  valid_types = names(assay_to_featuretype)
  assertthat::assert_that(all(type %in% valid_types),
                          msg=FormatMessage("'{type} must be: {valid_types*}."))
  converted_type = assay_to_featuretype[type]
...
ReadMetrics_10x = function(metrics_file) {
  ...
  if (metrics_table_colnms[1] == "Category") {
    ...
    metrics_nms = names(metrics_table) = assays_10x[]
      
      dplyr::case_when(
      names(metrics_table) == "Gene Expression" ~ "RNA",
      names(metrics_table) == "Multiplexing Capture" ~ "MPLX",
      names(metrics_table) == "VDJ T" ~ "VDJT",
      names(metrics_table) == "VDJ B" ~ "VDJB",
      names(metrics_table) == "Antibody Capture" ~ "ADT",
      names(metrics_table) == "Chromatin Accessibility" ~ "ATAC",
      names(metrics_table) == "CRISPR Guide Capture" ~ "CRISPR",
      names(metrics_table) == "Antigen Capture" ~ "BEAM",
      names(metrics_table) == "Custom" ~ "CUSTOM",
