...
highestExpr_summary = purrr::map_dfr(SeuratObject::Layers(sc, "counts"), function(l) {
  ...
  top_perc = t(t(top_counts)/total_counts)*100
...
p = Seurat::FeatureScatter(sc, feature1="pXIST_RNA", feature2="pChrY_RNA", shuffle=TRUE, plot.cor=FALSE, raster=ncol(sc)>=getOption("raster.threshold"))
p = p + AddPlotStyle()
print(p)

plist = Seurat::VlnPlot(sc, features=c("pXIST_RNA", "pChrY_RNA"), pt.size=0, raster=FALSE, combine=FALSE)
plist = purrr::map(plist, function(p) return(p + AddPlotStyle()))
print(plist)
```
