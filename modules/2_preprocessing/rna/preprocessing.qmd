...
# Make plots
plist = Seurat::VlnPlot(sc, features=qc_features, combine=FALSE, pt.size=0, raster=ncol(sc)>=getOption("raster.threshold"))
...
plist = purrr::map(qc_feature_cors, function(c) {
  ...
  if (ncol(sc)>=getOption("raster.threshold")) {
    p = p + ggrastr::rasterize(geom_point(), dpi=getOption("ggrastr.default.dpi"), dev=getOption("ggrastr.default.dev"))
  } else {
    p = p + geom_point()
  }
  p = p + AddStyle()
