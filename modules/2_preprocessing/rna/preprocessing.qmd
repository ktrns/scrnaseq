...
sc = Seurat::FindVariableFeatures(sc, assay=default_assay, verbose=FALSE, selection.method="vst")

# Make a subset/sketch of the default assay in Seurat
# Note: assay is saved in memory
sc = Seurat::SketchData(sc, 
                       assay=default_assay, 
                       ncells=min(5000, length(Seurat::Cells(sc))),
                       sketched.assay=paste0(default_assay, "Sketch"))

# Run a "quick" single-cell analysis on the sketch assay
sc = Seurat::NormalizeData(sc, assay=paste0(default_assay, "Sketch"), normalization.method="LogNormalize", scale.factor=10000, verbose=FALSE)
sc = Seurat::FindVariableFeatures(sc, verbose=FALSE, assay=paste0(default_assay, "Sketch"), selection.method="vst")
sc = Seurat::ScaleData(sc, verbose=FALSE, assay=paste0(default_assay, "Sketch"))
sc = Seurat::RunPCA(sc, verbose=FALSE, assay=paste0(default_assay, "Sketch"))
default_reduction = "pca"

num_datasets = length(SeuratObject::Layers(sc, assay=paste0(default_assay, "Sketch"), search="data."))
if (num_datasets > 1) {
  sc = Seurat::IntegrateLayers(sc, 
                               method=Seurat::CCAIntegration, 
                               orig.reduction="pca",
                               new.reduction="integrated.cca", 
                               assay=paste0(default_assay, "Sketch"),
                               verbose=FALSE)
  default_reduction = "integrated.cca"
}

sc = Seurat::FindNeighbors(sc, reduction=default_reduction, dims=1:10, assay=paste0(default_assay, "Sketch"))
sc = Seurat::FindClusters(sc, resolution=0.5, verbose=FALSE)
sc = Seurat::RunUMAP(sc, dims=1:10, reduction=default_reduction, assay=paste0(default_assay, "Sketch"))
...
sc[[]][paste("pMito", default_assay, sep="_")] = Seurat::PercentageFeatureSet(sc, assay=default_assay, features=mt_features)
...
sc[[paste("pRibosomal", default_assay, sep="_")]] = Seurat::PercentageFeatureSet(sc, assay=default_assay, features=ribo_features)
...
sc[[paste("pGlobin", default_assay, sep="_")]] = Seurat::PercentageFeatureSet(sc, assay=default_assay, features=globin_features)

# Calculate percentage of counts in globin genes
ercc_features = grep("^ERCC-", x=rownames(feature_metadata), value=TRUE, ignore.case=TRUE)
sc[[paste("pERCC", default_assay, sep="_")]] = Seurat::PercentageFeatureSet(sc, assay=default_assay, features=ercc_features)
...
if ("ensembl_chr" %in% colnames(feature_metadata)) {
  ...
  sc[[paste("pChrY", default_assay, sep="_")]] = Seurat::PercentageFeatureSet(sc, assay=default_assay, features=chrY_features)
}
```
...
# Make ridge plots
plist2 = Seurat::RidgePlot(sc, features=barcode_qc, group.by="seurat_clusters", combine=FALSE, assay=default_assay)
...
layers = SeuratObject::Layers(sc, assay=default_assay, search="^data.")
      ...
      assay = "RNA"
        ...
        if (isTRUE(x = move)) {
            for (i in seq_len(length.out = nrow(x = df))) {
                pth <- df$path[i]
                df[i, "path"] <- as.character(x = .FileMove(path = pth, 
                  new_path = destdir))
            }
        }
