...
# Prepare captions
captions = GeneratePlotCaptions(names(plist), remove=paste0(default_assay, "_"), split="__")
...
# Split sc Seurat object by orig.ident.
# For each orig.ident, run normalisation, PCA, clustering and UMAP.
# Use furrr
sc_list = SplitObject(sc, split.by="orig.ident")
sc_list = furrr::future_map(sc_list, function(x) {
  # Normalise
  x = Seurat::NormalizeData(x, assay=default_assay, normalization.method="LogNormalize", scale.factor=10000, verbose=FALSE)
  
  # Run PCA
  x = Seurat::RunPCA(x, assay=default_assay, verbose=FALSE)
  
  # Run clustering
  x = Seurat::FindNeighbors(x, reduction="pca", dims=1:20, verbose=FALSE)
  x = Seurat::FindClusters(x, resolution=0.5, verbose=FALSE)
  
  # Run UMAP
  x = Seurat::RunUMAP(x, reduction="pca", dims=1:20, verbose=FALSE)
  
  return(x)
})

#Then run normalisation, PCA, clustering and UMAP for each.
...
# Save Seurat object
saveRDS(sc, file=file.path(module_dir, "sc", "sc.rds"))
```
