...
for (a in Seurat::Assays(sc)) {
  ...
  f = paste(c("nCount", "nFeature", "pCountsTop50"), a, sep="_")
  captions = paste(a, c("Number of counts per barcode", "Number of features per barcode", "Percent counts in top50 features"), sep=": ")
  plist = VlnPlot(sc, features=f, pt.size=0, combine=FALSE, raster=FALSE)
  plist = purrr::map(plist,  function(p) return(p + AddPlotStyle(legend_position="none")))
  for (i in seq(plist)) {
...
SeuratObject::saveRDS(object=sc, file=file.path(module_dir, "sc", "sc.rds"), destdir=file.path(module_dir, "sc"))
```
