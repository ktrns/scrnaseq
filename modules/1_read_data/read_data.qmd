...
In this chapter, we read in counts data as "features x barcodes" matrices where features are typically genes (but can also be peaks, protein antibody, ...) and barcodes are typically cells (but can also be nuclei or spots). If provided, we also read in additional metadata for features and barcodes. For spatial data, we additionally import the image data. All data is saved as an Seurat object which is then processed in the next chapters.
...
barcode_metadata = purrr::map_dfr(seq_along(counts_lst), function(i) {
  ...
  bc_meta = attr(assay_counts_lst[[i]], "barcode_metadata") %>% tibble::rownames_to_column()
...
for (i in seq_along(counts_lst)) {
  ...
  barcodes = purrr::map_dfr(seq_along(counts_lst[[i]]), function(j) {
    bc_meta = attr(counts_lst[[i]][[j]], "barcode_metadata") %>% 
      tibble::rownames_to_column("new_barcode") %>% 
      dplyr::select(orig_barcode, new_barcode)
    return(bc_meta)
  }) %>% unique()
  barcodes = setNames(barcodes$new_barcode, barcodes$orig_barcode)
  j = match(barcodes, Cells(sc))
  barcodes = barcodes[order(j)]
  
  # Read image
  image = ReadImage(image_dir=image_dir, technology=technology, assay=default_assay, barcodes=barcodes)
...
for (a in Seurat::Assays(sc)) {
  ...
  for (l in SeuratObject::Layers(sc[[a]])) {
    ...
    counts_perc = BPCells::t(BPCells::t(counts)/total_counts)*100
    
    counts_perc_rnks = BPCells:::rank_transform(counts_perc, axis="col") %>%
      as("dgCMatrix")
    top50_cutoffs = counts_perc_rnks %>% 
      sparseMatrixStats::colMaxs()
    top50_cutoffs = top50_cutoffs - 50
    counts_perc_rnks_top50 = counts_perc_rnks < top50_cutoffs
    
    
    counts_perc_top50_rnks = counts_perc_top50_rnks > 50
    counts_perc_top50_rnks = BPCells::min_scalar(counts_perc_top50_rnks, 1)
    
    #counts_perc_top50_rnks = counts_perc_top50_rnks %>% as("dgCMatrix")
    
    counts_perc_top50 = BPCells:::mask_matrix(counts_perc, counts_perc_top50_rnks, invert=FALSE)
    
    
    BPCells::mask_matrix
    
    # Perc counts in top 50 genes
    col_indices = 1:ncol(counts_perc)
    chunk_size = 100
    ...
    top50_counts = purrr::map(chunks, function(c) {
      c = chunks[[1]]
      cts = counts_perc[, c] %>%
        BPCells:::rank_transform(axis="col")
      
      
        as("dgCMatrix")
      top50_cts = purrr::map_dbl(1:ncol(cts), function(i) {
        order(cts[, i, drop=TRUE], decreasing=TRUE) %>% 
          head(50) %>% 
          sum() %>%
          return()
      })
...
for (a in Seurat::Assays(sc)) {
  ...
  for (l in SeuratObject::Layers(sc[[a]])) {
    ...
    sc[[a]][paste("meanPerc", n, sep="_")] = counts_mean
  }
}
```
