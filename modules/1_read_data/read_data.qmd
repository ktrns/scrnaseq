  ...
  for(j in seq_along(counts_lst[[i]])) {
    ...
    if (on_disk_counts) {
      ...
      WriteCounts(counts=counts_lst[[i]][[j]], 
                  path=file.path(module_dir, "tmp", paste(experiment, assay, sep=".")), 
                  format="matrix_directory", 
                  overwrite=param("on_disk_counts_overwrite"))
      
      # Then open matrix directory for analysis
      counts_lst[[i]][[j]] = BPCells::open_matrix_dir(file.path(module_dir, "tmp", paste(experiment, assay, sep=".")))
    } else {
...
for (a in Seurat::Assays(sc)) {
  ...
  for (l in SeuratObject::Layers(sc[[a]], "counts")) {
    ...
    with_progress({
      top50_counts = SumTopN(counts, n=50, margin=2, cores=1, chunk_size=5000)
    })
...
for (a in Seurat::Assays(sc)) {
  ...
  for (l in SeuratObject::Layers(sc[[a]], "counts")) {
    ...
    with_progress({
      counts_median = CalculateMedians(counts_perc, margin=1, cores=1, chunk_size=1000)
    })
...
# Save Seurat object: SeuratObject::SaveSeuratRds does not work
saveRDS(sc, file=file.path(module_dir, "sc", "sc.rds"))
#SeuratObject::SaveSeuratRds(sc,
#              file=file.path(module_dir, "sc", "sc.rds"),
#              destdir=file.path(module_dir, "sc"))

# Do not delete since SeuratObject::SaveSeuratRds does not copy the on-disk data directories properly
# Clean tmp directory
