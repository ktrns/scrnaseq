...
Raw data from single-cell experiments needs to be converted into count matrices. For example, for scRNA-seq, such a count matrix contains expression values per cell and gene. More generally spoken, a count matrix contains measurements per barcode (e.g. cell, nuclei or spot) and feature (e.g. gene, protein or ATAC peak) where barcodes correspond to columns and features correspond to rows (but vice versa is also possible).
...
: Example of a counts matrix for scRNA-seq data
:::
:::

How to construct such a count matrix depends on the experimental protocol used for the single-cell experiment. Here we roughly outline the steps needed for the protocols for officially supported -  Smartseq2, Smartseq3, 10x, 10x Visium, 10x Xenium, Parse Biosciences and Scale Bio - and describe their relevant output files.
...
for (i in seq(counts_lst)) {
  for(j in seq_along(counts_lst[[i]])) {
    ...
    if (on_disk_counts) {
      ...
      #counts_lst[[i]][[j]] = BPCells::open_matrix_dir(file.path(on_disk_path, paste(assay, "counts", experiment, sep=".")))
...
sc = ScAddColours(sc, colours=list(
  ...
  seurat_clusters=setNames(godsnot_102[1:length(godsnot_102)], 1:length(godsnot_102))
))
