...
for (a in Seurat::Assays(sc)) {
  ...
  plist = VlnPlot(sc, features=f, pt.size=0, combine=FALSE, raster=ncol(sc)>=getOption("raster.threshold"))
  plist = purrr::map(plist,  function(p) return(p + AddStyle(legend_position="none")))
  for (i in seq(plist)) {
