...
# Print datasets
rmarkdown::paged_table(datasets)
```
...
rmarkdown::paged_table(counts_summary)
```
...
```{r}
#| label: barcode_metadata
#| timeit: null

sc[[]] %>% head(5) %>% rmarkdown::paged_table()
```
...
for(a in Seurat::Assays(sc)) {
  cat('#### ',a,'\n \n')
  df = sc[[a]][] %>% head(5) %>% rmarkdown::paged_table()
