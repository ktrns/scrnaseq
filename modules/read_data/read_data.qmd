...
params:
  ...
  # - rename_features_ensembl (optional): By default, ids are used to name features (first column). For gene features, set to '1' to fetch gene names (and more annotation)
  #                                       from Ensembl. Set to '0' or leave empty to deactivate. If multiple assays are read, this option can be specified either once or 
  #                                       for all assays. If used together with rename_features_metadata, then rename_features_metadata must be set '1' for all assays 
  #                                       that should be renamed with rename_features_ensembl.
  # - rename_features_metadata (optional): By default, ids are used to name features (first column). This column controls how to rename the features using metadata. 
  #                                        Can be the name or index of a feature  metadata column. If multiple assays are read, this option can be specified either once 
  #                                        or for all assays. If used together with rename_features_ensembl, then rename_features_metadata must be set '1' for all assays 
  #                                        that should be renamed with rename_features_ensembl.
...
for (i in 1:nrow(datasets)) {
  ...
  # Set sample, path, technology and assays
  ...
  # Read feature metadata
  rename_features_metadata = datasets$rename_features_metadata[i]
  if (is.na(rename_features_metadata)) {
    rename_features_metadata = NULL
  } else {
    feature_metadata_files = strsplit(rename_features_metadata, split=",")  %>% unlist() %>% trimws()
  }
