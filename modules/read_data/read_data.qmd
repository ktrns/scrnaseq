...
params:
  ...
  datasets_file: "datasets/10x_pbmc_atac.xlsx"
...
# Demonstration: write to 'results' directory, will then provided with the final report
write.table(datasets, file=file.path(module_dir, "results", "datasets.txt"))
  ...
  for(j in seq_along(counts_lst[[i]])) {
    ...
    if (param("on_disk_counts")){
      ...
      WriteCounts(counts=counts_lst[[i]][[j]], 
                  path=file.path(module_dir, "counts", experiment, assay), 
                  format="matrix_directory", 
                  overwrite=param("on_disk_counts_overwrite"))
      
      # Then open matrix directory for analysis
      counts_lst[[i]][[j]] = BPCells::open_matrix_dir(file.path(module_dir, "counts", experiment, assay))
    } else {
    ...
    attr(counts_lst[[i]][[j]], "dataset") = path
  }
...
# Collect barcode metadata (from all assays)
...
note: quarto allows to hide content - use here ?
...
for (i in seq_along(counts_lst)) {
  ...
  if (technology == "10x_visium") {
    ...
    image_dir = file.path(path, "spatial")
