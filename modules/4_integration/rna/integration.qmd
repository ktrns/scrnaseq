...
if (length(integration_method) > 0) {
  # Integration requested and neccessary

  # Do we need to do sketching?
  if (num_barcodes > min_barcodes_for_sketching) {
    ...
    # SCTransform normalisation needs to be repeated for the sketch?
    if(normalisation_method == "SCT") {
      sc = SCTransform(sc, 
                   new.assay.name=SeuratObject::DefaultAssay(sc),
                   vst.flavor="v2",
  ...
  sc = Seurat::RunPCA(sc, seed.use=getOption("random_seed"))
  
  # Integrate
} else if (length(integration_method) == 0) {
